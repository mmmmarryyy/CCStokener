...
public class CombineResourcesUtil {
    ...
    private static String buildUrl(URL base, String path) {
        ...
        if (c == '/') {
            return base.getProtocol() + "://" + base.getHost() + (base.getPort() != 80 && base.getPort() != -1 ? ":" + base.getPort() : "") + path;
        } else {
            return base + "/../" + path;
        }
    }
    ...
    private static int getType(Node node) {
        if (node instanceof Element) {
            ...
            if (element.getTagName().equalsIgnoreCase("link") && element.getAttribute("rel").equalsIgnoreCase("stylesheet") && element.getAttribute("type").equalsIgnoreCase("text/css") && element.getAttribute("charset").equalsIgnoreCase("utf-8")) {
                return 1;
            }
            if (element.getTagName().equalsIgnoreCase("script") && element.getAttribute("type").equalsIgnoreCase("text/javascript") && element.getAttribute("src") != null && element.getAttribute("src").length() > 0) {
                return 2;
            }
        }
        return -1;
