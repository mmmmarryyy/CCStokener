...
public final class XSLTC {
    ...
    public boolean compile(Vector stylesheets) {
        ...
        if (count == 1) {
            ...
            if (url instanceof URL) return compile((URL) url); else return false;
        } else {
    ...
    public byte[][] compile(String name, InputSource input, int outputType) {
        ...
        if (compile(input, name)) return getBytecodes(); else return null;
    }
    ...
    public void setClassName(String className) {
        ...
        if (_packageName == null) _className = name; else _className = _packageName + '.' + name;
    }
    ...
    private File getOutputFile(String className) {
        if (_destDir != null) return new File(_destDir, classFileName(className)); else return new File(classFileName(className));
    }
    ...
    public void setJarFileName(String jarFileName) {
        ...
        if (jarFileName.endsWith(JAR_EXT)) _jarFileName = jarFileName; else _jarFileName = jarFileName + JAR_EXT;
    ...
    public int registerAttribute(QName name) {
        ...
        if (code == null) {
            ...
            if ((uri != null) && (!uri.equals(""))) _namesIndex.addElement(uri + ":" + local); else _namesIndex.addElement(local);
        ...
        try {
            switch(_outputType) {
                ...
                case CLASSLOADER_OUTPUT:
                    ...
                    if (_outputType == BYTEARRAY_AND_FILE_OUTPUT) clazz.dump(new BufferedOutputStream(new FileOutputStream(getOutputFile(clazz.getClassName())))); else if (_outputType == BYTEARRAY_AND_JAR_OUTPUT) _bcelClasses.addElement(clazz);
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
