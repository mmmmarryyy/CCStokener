...
public class PagesExt extends Pages {
    ...
    protected void noConversation() {
        ...
        FacesMessages.instance().addFromResourceBundleOrDefault(StatusMessage.Severity.WARN, "org.jboss.seam.NoConversation", "The conversation ended, timed out or was processing another request");
    }
    ...
    private static boolean callAction(FacesContext facesContext) {
        ...
        String outcome = facesContext.getExternalContext().getRequestParameterMap().get("actionOutcome");
    ...
    public List<ResourceBundle> getResourceBundles(String viewId) {
        PageResourceBundles pageResourceBundles = PageResourceBundles.instance();
        List<ResourceBundle> result = pageResourceBundles.get(viewId);
        if (result == null) {
            result = new ArrayList<ResourceBundle>(1);
            List<Page> stack = getPageStack(viewId);
            for (int i = stack.size() - 1; i >= 0; i--) {
                Page page = stack.get(i);
            ...
            pageResourceBundles.put(viewId, result);
        }
        return result;
    }
    ...
    public static String getSuffix() {
        String defaultSuffix = FacesContext.getCurrentInstance().getExternalContext().getInitParameter(ViewHandler.DEFAULT_SUFFIX_PARAM_NAME);
    ...
    private static void parseNavigationHandler(Element element, Rule rule) {
        ...
        if (render != null) {
            ...
            Severity severity = severityName == null ? FacesMessage.SEVERITY_INFO : getFacesMessageValuesMap().get(severityName.toUpperCase());
        ...
        if (redirect != null) {
            ...
            String severityName = messageElement == null ? null : messageElement.attributeValue("severity");
            Severity severity = severityName == null ? FacesMessage.SEVERITY_INFO : getFacesMessageValuesMap().get(severityName.toUpperCase());
            rule.addNavigationHandler(new RedirectNavigationHandler(stringValueExpressionFor(viewId), stringValueExpressionFor(url), params, message, severity, control, includePageParams));
        }
