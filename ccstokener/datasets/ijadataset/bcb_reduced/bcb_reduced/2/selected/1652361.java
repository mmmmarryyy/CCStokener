...
public class FontReader extends DefaultHandler {
    ...
    public void setFontEmbedPath(URL path) {
        if (isCID) multiFont.embedFileName = path; else singleFont.embedFileName = path;
    }
    ...
    public void useKerning(boolean kern) {
        if (isCID) multiFont.useKerning = true; else singleFont.useKerning = true;
    }
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        if (localName.equals("font-metrics")) {
            ...
            if (isCID) {
                ...
                if (filename != null) {
                    try {
                        multiFont.embedFileName = URLBuilder.buildURL(Configuration.getFontBaseURL(), filename);
                ...
                if (filename != null) {
                    try {
                        singleFont.embedFileName = URLBuilder.buildURL(Configuration.getFontBaseURL(), filename);
                    } catch (java.net.MalformedURLException mfue) {
                        throw new SAXException(mfue);
                    }
                }
            ...
            if (isCID) multiFont.kerning.put(new Integer(attributes.getValue("kpx1")), currentKerning); else singleFont.kerning.put(new Integer(attributes.getValue("kpx1")), currentKerning);
            ...
            try {
                singleFont.width[Integer.parseInt(attributes.getValue("idx"))] = Integer.parseInt(attributes.getValue("wdt"));
            } catch (NumberFormatException ne) {
                System.out.println("Malformed width in metric file: " + ne.getMessage());
            }
        } else if ("pair".equals(localName)) {
            currentKerning.put(new Integer(attributes.getValue("kpx2")), new Integer(attributes.getValue("kern")));
        }
    }
