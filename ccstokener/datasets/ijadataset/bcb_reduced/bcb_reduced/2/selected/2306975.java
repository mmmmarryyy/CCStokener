...
public class PackageFragmentRootMetric extends AbstractMetricElement {
    ...
    public PackageFragmentRootMetric(String handle, BundleAnalyzer analyzer, IProgressMonitor monitor) {
        super(handle, analyzer);
        if (getJavaElement() instanceof JarPackageFragmentRoot) loadClassFilesFromJar();
        if (handle != null) initChildren(monitor);
            ...
            for (int i = 0; i < children.length; i++) {
                if (children[i] instanceof IPackageFragment) {
                    if (((IPackageFragment) children[i]).containsJavaResources()) {
                        AbstractMetricElement next = new PackageFragmentMetric(children[i].getHandleIdentifier(), analyzer, null);
                        if (next != null) {
                            addChild(next);
                        } else System.out.println("metrics for package " + children[i].getHandleIdentifier() + " not found.");
                    }
                }
                monitor.worked(1);
            }
