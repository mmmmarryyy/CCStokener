...
public class AddePointDataReader {
    ...
    public int[][] getData(int order) throws AddeException {
        if (status < 0) throw new AddeException("No data available");
        ...
        for (int i = 0; i < numParams; i++) {
            if (parameter.equalsIgnoreCase(params[i])) {
                ...
                if (units[i].equalsIgnoreCase("CHAR")) {
                    ...
                    return vals;
                } else if (ScalingFactors[i] != 0) {
                    double[][] vals = new double[1][iData[i].length];
                    for (int j = 0; j < iData[i].length; j++) {
                        vals[0][j] = iData[i][j] / Math.pow(10.0, (double) ScalingFactors[i]);
                    }
                    return vals;
                } else {
    ...
    public String toString() {
        if (status < 0) return new String("No data Available");
        ...
        for (int i = 0; i < iData[0].length; i++) {
            for (int j = 0; j < numParams; j++) {
                ...
                } else if (ScalingFactors[j] != 0) {
                    buf.append(iData[j][i] == McIDASUtil.MCMISSING ? "     " : Double.toString(iData[j][i] / Math.pow(10.0, (double) ScalingFactors[j])));
                } else {
                    buf.append(iData[j][i] == McIDASUtil.MCMISSING ? "     " : Integer.toString(iData[j][i]));
                }
                buf.append("\t");
            }
            buf.append("\n");
