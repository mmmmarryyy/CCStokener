...
public class RTSI {
    ...
    static {
        if (VERBOSE) System.err.println(RTSI.class.getName() + ": VERBOSE ON");
    }
    ...
    protected static URL getURL(String classpathPart, String pckgname) {
        ...
        try {
            ...
            if (classpathFile.isDirectory()) {
                ...
                if (file.exists()) urlStr = "file:" + classpathPart + pckgname;
            } else {
                jarfile = new JarFile(classpathPart);
                enm = jarfile.entries();
                pckgnameTmp = pckgname.substring(1);
        ...
        if (VERBOSE) System.out.println("Classpath " + classpathPart + ", package " + pckgname + " -> " + result);
        return result;
    }
   ...
   */
     ...
     */
        private int charGroup(char c) {
            int result;
            result = 0;
            if ((c >= 'a') && (c <= 'z')) result = 2; else if ((c >= '0') && (c <= '9')) result = 1;
            return result;
        }
            ...
            for (i = 0; i < s1.length(); i++) {
                if (s1.charAt(i) == s2.charAt(i)) {
                    ...
                    if (v1 != v2) {
                        ...
                        if (s1.charAt(i) < s2.charAt(i)) result = -1; else result = 1;
                    }
                    break;
                }
            }
