...
class ProjectConverter extends SwingWorker<Void, Map.Entry<LogLevelWrapper, String>> {
    ...
    ProjectConverter(MainFrame frame) {
        if (null == (_frame = frame)) throw new IllegalStateException("No main frame instance provided");
    }
    ...
    protected Void doInBackground() throws Exception {
        ...
        if ((root == null) || (root.length() <= 0)) throw new IllegalStateException("No root folder available");
        if (convertProject(new File(root), f.isRecursiveScanning())) info("Done"); else info("Cancelled");
        return null;
    }
    ...
    private boolean convertProject(final File rootFolder, final boolean recursive) {
        ...
        if (rootFolder.isDirectory()) {
            if ("target".equalsIgnoreCase(name) || (name.charAt(0) == '.')) return !isCancelled();
        }
        ...
        if (!recursive) return !isCancelled();
        final File[] files = rootFolder.listFiles();
        for (final File f : files) {
            if (isCancelled()) return false;
            if (!f.isDirectory()) continue;
            if (!convertProject(f, recursive)) return false;
        }
        return !isCancelled();
    }
    ...
    private boolean convertEclipseSettings(final File rootFolder, final boolean webProject) {
        ...
        if (!checkEclipseSettingsFiles(settingsFolder, "org.eclipse.m2e.core.prefs", "org.maven.ide.eclipse.prefs")) return false;
        if (webProject) return checkEclipseSettingsFiles(settingsFolder, createProjectNameProps(settingsFolder), "org.eclipse.wst.common.component", "org.eclipse.wst.common.project.facet.core.xml");
        return true;
    }
    ...
    private boolean checkEclipseSettingsFiles(final File settingsFolder, final Map<String, String> propsMap, final String... names) {
        ...
        for (final String fileName : names) {
            ...
            if (!copyUpdateFile(file, "settings/" + fileName, propsMap)) return false;
        }
        return true;
    }
    ...
    private boolean convertEclipseClasspath(final File rootFolder, final boolean webProject) {
        ...
        if (!classpathFile.exists()) {
            if (!createDefaultClasspath(classpathFile)) return false;
        }
        ...
        try {
            final Document doc = (Document) DOMUtils.loadDocument(classpathFile).cloneNode(true);
            final Element root = doc.getDocumentElement();
            final Collection<? extends Element> entries = convertClasspathFile(doc, root, root.getChildNodes(), webProject);
            ...
            error("Failed (" + e.getClass().getSimpleName() + ")" + " to parse " + classpathFile.getAbsolutePath() + ": " + e.getMessage());
            return false;
        }
    }

    private Collection<Element> convertClasspathFile(final Document doc, final Element root, final NodeList nodes, final boolean webProject) {
        final int numNodes = (nodes == null) ? 0 : nodes.getLength();
        Collection<String> containers = null;
        for (int index = 0; index < numNodes; index++) {
            final Node n = nodes.item(index);
            if ((n == null) || (n.getNodeType() != Node.ELEMENT_NODE)) continue;
            final Element elem = (Element) n;
            final String tagName = elem.getTagName();
            if (!ClasspathUtils.CLASSPATHENTRY_ELEM_NAME.equalsIgnoreCase(tagName)) continue;
            final Map.Entry<String, String> conInfo = ClasspathUtils.getContainerEntryInfo(elem);
            final String kind = (conInfo == null) ? null : conInfo.getKey(), path = (conInfo == null) ? null : conInfo.getValue();
            if (!ClasspathUtils.CON_ENTRY_KIND.equalsIgnoreCase(kind)) continue;
            if ((path == null) || (path.length() <= 0)) continue;
            final int sepPos = path.indexOf('/');
            final String name = (sepPos < 0) ? path : path.substring(0, sepPos);
            if (containers == null) containers = new TreeSet<String>(String.CASE_INSENSITIVE_ORDER);
            if (!containers.add(name)) continue;
        }
        if ((containers != null) && containers.contains("org.eclipse.m2e.MAVEN2_CLASSPATH_CONTAINER")) return Collections.emptyList();
        final Element elem = ClasspathUtils.createContainerEntry(doc, "org.eclipse.m2e.MAVEN2_CLASSPATH_CONTAINER");
        if (webProject) ClasspathUtils.addWebProjectDependency(doc, elem);
    ...
    private boolean convertEclipseProject(final File rootFolder, final boolean webProject) {
        ...
        if (!projFile.exists()) {
            if (!createEclipseProject(projFile)) return false;
        }
        if (!projFile.isFile()) return false;
        try {
            final Document doc = (Document) DOMUtils.loadDocument(projFile).cloneNode(true);
            final Element rootElem = doc.getDocumentElement();
            final Collection<? extends Element> cmdBuild = checkBuildSpecs(doc, rootElem.getElementsByTagName(ProjectUtils.BUILD_SPEC_ELEM_NAME), webProject), natures = checkProjectNatures(doc, rootElem.getElementsByTagName(ProjectUtils.NATURES_ELEM_NAME), webProject);
            if (((cmdBuild != null) && (cmdBuild.size() > 0)) || ((natures != null) && (natures.size() > 0))) {
                debug("Converting " + projFile.getAbsolutePath());
                PrettyPrintTransformer.DEFAULT.transform(doc, projFile);
            }
            return true;
        } catch (Exception e) {
            error("Failed (" + e.getClass().getSimpleName() + ")" + " to parse " + projFile.getAbsolutePath() + ": " + e.getMessage());
            return false;
        }
    }
    ...
    private Collection<Element> checkBuildSpecs(final Document doc, final NodeList nodes, final boolean webProject) {
        ...
        final int numNodes = (nodes == null) ? 0 : nodes.getLength();
        if (numNodes > 1) throw new UnsupportedOperationException("Too many build spec elements: " + numNodes);
        for (int index = 0; index < numNodes; index++) {
            final Node n = nodes.item(index);
            if ((n == null) || (n.getNodeType() != Node.ELEMENT_NODE)) continue;
            final Element elem = (Element) n;
            final Collection<Element> cmds = checkBuildCommands(doc, elem, elem.getElementsByTagName(ProjectUtils.BUILD_CMD_ELEM_NAME), webProject);
            if ((cmds == null) || (cmds.size() <= 0)) continue;
            if (result == null) result = new LinkedList<Element>(cmds); else result.addAll(cmds);
        }
        return result;
    }
    ...
    private Collection<Element> checkBuildCommands(final Document doc, final Element parent, final NodeList nodes, final boolean webProject) {
        ...
        for (int index = 0; index < numNodes; index++) {
            ...
            for (int childIndex = 0; childIndex < numChildren; childIndex++) {
                ...
                final String tagName = elem.getTagName();
                if (!ProjectUtils.BUILD_CMD_NAME_TAG.equalsIgnoreCase(tagName)) continue;
                final String tagValue = DOMUtils.getElementStringValue(elem);
                if ((tagValue == null) || (tagValue.length() <= 0)) continue;
                if (names == null) names = new TreeSet<String>(String.CASE_INSENSITIVE_ORDER);
                if (!names.add(tagValue)) continue;
            }
        }
        Collection<Element> result = checkProjectBuilders(doc, parent, names, "org.eclipse.jdt.core.javabuilder", "org.eclipse.m2e.core.maven2Builder");
        if (webProject) {
            final Collection<Element> elemList = checkProjectBuilders(doc, parent, names, "org.eclipse.wst.common.project.facet.core.builder", "org.eclipse.wst.validation.validationbuilder");
            if ((result == null) || (result.size() <= 0)) result = elemList; else if ((elemList != null) && (elemList.size() > 0)) result.addAll(elemList);
        }
        return result;
    }

    private List<Element> checkProjectBuilders(final Document doc, final Element parent, final Collection<String> names, final String... builders) {
        if ((builders == null) || (builders.length <= 0)) return Collections.emptyList();
        ...
        for (final String name : builders) {
            ...
            if (result == null) result = new ArrayList<Element>(builders.length);
            result.add(elem);
        }
        return result;
    }
    ...
    private Collection<Element> checkProjectNatures(final Document doc, final NodeList nodes, final boolean webProject) {
        final int numNodes = (nodes == null) ? 0 : nodes.getLength();
        if (numNodes > 1) throw new UnsupportedOperationException("Too many natures elements: " + numNodes);
        Collection<Element> result = null;
        for (int index = 0; index < numNodes; index++) {
            final Node n = nodes.item(index);
            if ((n == null) || (n.getNodeType() != Node.ELEMENT_NODE)) continue;
            final Element elem = (Element) n;
            final String tagName = elem.getTagName();
            if (!ProjectUtils.NATURES_ELEM_NAME.equalsIgnoreCase(tagName)) continue;
            final Collection<String> natures = ProjectUtils.getNatures(elem);
            Collection<Element> natList = checkProjectNatures(doc, elem, natures, "org.eclipse.m2e.core.maven2Nature", "org.eclipse.jdt.core.javanature");
            if (webProject) {
                final Collection<Element> webList = checkProjectNatures(doc, elem, natures, "org.eclipse.wst.common.modulecore.ModuleCoreNature", "org.eclipse.wst.common.project.facet.core.nature");
                if ((natList == null) || natList.isEmpty()) natList = webList; else if ((webList != null) && (webList.size() > 0)) natList.addAll(webList);
            }
            if ((natList == null) || natList.isEmpty()) continue;
            if (result == null) result = natList; else result.addAll(natList);
        }
        return result;
    }

    private List<Element> checkProjectNatures(final Document doc, final Element parent, final Collection<String> natures, final String... names) {
        if ((names == null) || (names.length <= 0)) return Collections.emptyList();
        List<Element> result = null;
        for (final String name : names) {
            if ((natures != null) && natures.contains(name)) continue;
            final Element natureElem = ProjectUtils.createNature(doc, name);
            parent.appendChild(natureElem);
            if (result == null) result = new ArrayList<Element>(names.length);
    ...
    private static File checkMavenProject(final File file) {
        ...
        final File srcFolder = exists(file, "src"), mainFolder = exists(srcFolder, "main"), javaFolder = exists(mainFolder, "java");
        if ((javaFolder == null) || (!javaFolder.isDirectory())) return null;
        return mainFolder;
    }

    private static File exists(final File folder, final String fileName) {
        if ((folder == null) || (!folder.isDirectory()) || (fileName == null) || (fileName.length() <= 0)) return null;
    ...
    private Map.Entry<LogLevelWrapper, String> log(LogLevelWrapper l, String msg) {
        if ((l == null) || (msg == null) || (msg.length() <= 0)) return null;
        final Map.Entry<LogLevelWrapper, String> msgEntry = new MapEntryImpl<LogLevelWrapper, String>(l, msg);
        publish(msgEntry);
        return msgEntry;
    }
    ...
    protected void process(List<Map.Entry<LogLevelWrapper, String>> chunks) {
        ...
        final MainFrame f = getMainFrame();
        if (f == null) return;
        for (final Map.Entry<LogLevelWrapper, String> msgEntry : chunks) f.log(msgEntry.getKey(), msgEntry.getValue());
    }

    @Override
    protected void done() {
        final MainFrame f = getMainFrame();
        if (f != null) f.signalConversionDone(this);
    }
}
