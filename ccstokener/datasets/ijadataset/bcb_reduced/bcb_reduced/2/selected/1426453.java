...
public class OcAlfrescoServerUtility {
     ...
     * solr.
     */
    private static Properties solrProperties = OCSolrServer.getInstance().getApplicationProps();
    ...
    private String getToken() {
        ...
        try {
            ...
            alfresco = new URL(alfrescoUrl + alfrescoLogin + "?u=" + alfrescoUser + "&pw=" + alfrescoPassword);
            BufferedReader in = null;
            in = new BufferedReader(new InputStreamReader(alfresco.openStream()));
    ...
    private static String createDir() {
        String path = solrProperties.getProperty(SolrPropertyKeys.SOLR_TMP_DIR) + Thread.currentThread().getId();
    ...
    public static String deleteDir(final long idtrhread) {
        String path = solrProperties.getProperty(SolrPropertyKeys.SOLR_TMP_DIR) + idtrhread;
        File dir = new File(path);
