...
public class ResourceFinder implements Serializable {
    ...
    public ResourceFinder(String name) throws IOException {
        this.name = name;
   ...
   */
    ...
    private boolean tryFileLoader(String filename) {
        int i = filename.lastIndexOf(System.getProperty("file.separator"));
        String[] paths = { filename.substring(0, i) };
        file = searchDirectories(paths, filename.substring(i));
        return (file != null);
    }
    ...
    private boolean tryLoader(String name) {
        name = "/" + name;
        URL res = ResourceFinder.class.getResource(name);
        if (res == null) {
            return false;
        }
