package at.ac.ait.enviro.dssos.source;
...
import at.ac.ait.enviro.dssos.sink.TemplateWriter;
...
public abstract class AbstractSOSConnector implements ISOSConnector {
     ...
     * Sends a request to a SOS. The request is sent to the operation specific URL,
     * listed in the CapabilitiesDocument.
     * @param operationName The name of the SOS operation that should be performed.
     * @param request
     * @return The response as InputStream
     * @throws java.io.IOException in case of an error
     */
    protected InputStream sendRequest(String operationName, TemplateWriter request) throws SosClientException {
        try {
            URL targetURL = getOperationURL(operationName);
            HttpURLConnection connection = doConnect(targetURL, POST_REQUEST);
            BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(connection.getOutputStream()));
            ...
            final String msg = String.format("error sending '%s'-request.", ex);
            log.error(msg);
            throw new SosClientException(msg, ex);
        }
    }
