...
/**
 * 功能描述：在Android手机上装个Gtalk，接收Log4j的异常提醒，蛮实用的想法。
...
public class GtalkAppender extends AppenderSkeleton {
    ...
    protected void append(LoggingEvent event) {
        ...
        if ((now - lastSendTime) > 1000) {
            String content = sdf.format(new Date()) + " " + event.getLevel() + " [" + event.getLoggerName() + "]\n" + event.getRenderedMessage() + "\n";
    ...
    private void sendMessage(String content) {
        ...
        try {
            ...
            String urlAddress = server + "/api/pushMsg?key=" + apikey + "&to=" + to + "&msg=" + content;
