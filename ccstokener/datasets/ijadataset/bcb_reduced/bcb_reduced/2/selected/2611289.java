...
public class GifImage {
    ...
    void process(InputStream is) throws IOException {
        ...
        readHeader();
        readContents();
        if (frames.size() == 0) throw new IOException("The file does not contain any valid image.");
    }
    ...
    protected void readHeader() throws IOException {
        String id = "";
        for (int i = 0; i < 6; i++) id += (char) in.read();
    ...
    protected int readBlock() throws IOException {
        blockSize = in.read();
        if (blockSize <= 0) return blockSize = 0;
    ...
    protected void readImage() throws IOException {
        ...
        if (transparency && transIndex >= m_curr_table.length / 3) transparency = false;
