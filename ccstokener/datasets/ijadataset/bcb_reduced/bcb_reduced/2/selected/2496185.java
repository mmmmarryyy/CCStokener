...
public final class Remote extends UnicastRemoteObject implements RemoteInvoke, Unreferenced {
    ...
    private static final class RSSF implements RMIServerSocketFactory {
        ...
        public ServerSocket createServerSocket(int port) throws IOException {
            ServerSocket ss = host == null ? RMISocketFactory.getDefaultSocketFactory().createServerSocket(this.port) : new ServerSocket(this.port, 50, InetAddress.getByName(host));
            if (host == null) {
    ...
    private static final class RCSF implements RMIClientSocketFactory, Serializable {
        ...
        private static final long serialVersionUID = 0x6060842L;
    ...
    public static void config(int serverPort, String clientHost, int clientPort, String proxyHost, int proxyPort, final String username, final String password) {
        ...
        try {
            if (proxyHost != null) {
                ...
                if (username != null) Authenticator.setDefault(new Authenticator() {
    ...
    */
    public static void shutdown() {
        synchronized (items) {
            for (int i = items.size() - 1; i >= 0; i--) try {
    ...
    */
    public static Class autobox(Class arg) {
        return arg.isPrimitive() ? arg == boolean.class ? Boolean.class : arg == byte.class ? Byte.class : arg == char.class ? Character.class : arg == short.class ? Short.class : arg == int.class ? Integer.class : arg == long.class ? Long.class : arg == float.class ? Float.class : Double.class : arg;
    }
    ...
    public static Method findBestMethod(Object item, String method, Class[] args) {
        ...
        if (methods != null) {
            ...
            if (arguments != null) {
                ...
                for (int i = 0; i < argumentSet.length; i++) if (Arrays.equals(args, (Class[]) argumentSet[i])) return (Method) arguments.get(argumentSet[i]);
            }
        }
        ...
        if (((Object[]) args).length > 0) {
            ...
            list: for (int i = 0; i < ms.length; i++) {
                if (ms[i].getName().equals(method) && ms[i].getParameterTypes().length == args.length) {
                    for (int j = 0; j < args.length; j++) if (args[j] != null && !autobox(ms[i].getParameterTypes()[j]).isAssignableFrom(args[j])) continue list;
                    matchList.add(ms[i]);
                }
            }
        ...
        if (best == null) {
            for (int i = 0, goodness = -1; i < matchList.size(); i++) {
                ...
                for (int j = 0; j < args.length; j++) if (args[j] != null && args[j].isAssignableFrom(autobox(m.getParameterTypes()[j]))) closeness++;
    ...
    */
    public static Object invoke(final Object item, final String method, final Object args) throws Exception {
        Object o_args[] = args instanceof Object[] ? (Object[]) args : args != null ? new Object[] { args } : NOARGS;
        if (item instanceof RemoteInvoke) {
            if (o_args.length == 0 && method.equals("hashCode")) return new Integer(item.hashCode());
            if (o_args.length == 1 && method.equals("equals")) return item.equals(o_args[0]) ? Boolean.TRUE : Boolean.FALSE;
            for (int i = 0; i < o_args.length; i++) {
                if (o_args[i] != null && !(o_args[i] instanceof Serializable)) {
                    HashSet interfaces = new HashSet();
                    for (Class c = o_args[i].getClass(); c != null; c = c.getSuperclass()) interfaces.addAll(Arrays.asList(c.getInterfaces()));
                    o_args[i] = gnu.cajo.utils.extra.TransparentItemProxy.getItem(o_args[i], (Class[]) interfaces.toArray(new Class[0]));
                }
            }
        }
        if (item instanceof Invoke) return ((Invoke) item).invoke(method, args);
        Class[] c_args = o_args != NOARGS ? new Class[o_args.length] : NULL;
        if (c_args != NULL) for (int i = 0; i < c_args.length; i++) c_args[i] = o_args[i] == null ? null : o_args[i].getClass();
        ...
        if (m != null) try {
            ...
            if (result != null && !(result instanceof Serializable)) try {
                ...
                HashSet interfaces = new HashSet();
                for (Class c = result.getClass(); c != null; c = c.getSuperclass()) interfaces.addAll(Arrays.asList(c.getInterfaces()));
                return gnu.cajo.utils.extra.TransparentItemProxy.getItem(result, (Class[]) interfaces.toArray(new Class[0]));
