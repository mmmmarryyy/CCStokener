...
public class JarURLConnectionTest extends junit.framework.TestCase {
    ...
    private static final String BASE = "file:resources/org/apache/harmony/luni/tests/java/net/lf.jar";
    ...
    public void test_getContentLength() throws Exception {
        ...
        assertEquals("Returned incorrect size for jar file", 33095, u.openConnection().getContentLength());
        u = new URL("jar:" + BASE + "!/plus.bmp");
        assertEquals("Returned incorrect size for the entry", 190, u.openConnection().getContentLength());
    }
    ...
    public void test_getContentType() throws Exception {
        URL u = new URL("jar:" + BASE + "!/");
        assertEquals("Returned incorrect type for jar file", "x-java/jar", u.openConnection().getContentType());
        u = new URL("jar:" + BASE + "!/plus.bmp");
        assertEquals("Returned incorrect type for the entry with known type", "image/bmp", u.openConnection().getContentType());
        u = new URL("jar:" + BASE + "!/Manifest.mf");
        assertEquals("Returned incorrect type for the entry with known type", "content/unknown", u.openConnection().getContentType());
    }
