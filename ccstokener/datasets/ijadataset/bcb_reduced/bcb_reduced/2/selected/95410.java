...
public class Dbf implements DbfConsts {
    ...
    class DbfFileHeader {
        ...
        private void getDbfFileHeader(EndianDataInputStream file) throws IOException {
            ...
            if (dbf_id == 3) hasmemo = true; else hasmemo = false;
    ...
    public Vector ParseRecord(StringBuffer rec) {
        ...
        for (int i = 0; i < numfields; i++) {
            ...
            if (DEBUG) logger.info(DBC + "" + t.substring(fielddef[i].fieldstart, fielddef[i].fieldstart + fielddef[i].fieldlen));
            switch(fielddef[i].fieldtype) {
                ...
                case 'N':
                case 'F':
                    if (fielddef[i].fieldnumdec == 0) try {
                        record.addElement(Integer.decode(t.substring(fielddef[i].fieldstart, fielddef[i].fieldstart + fielddef[i].fieldlen)));
                    } catch (java.lang.NumberFormatException e) {
                        record.addElement(new Integer(0));
                    } else try {
                        record.addElement(Float.valueOf(t.substring(fielddef[i].fieldstart, fielddef[i].fieldstart + fielddef[i].fieldlen)));
                    } catch (java.lang.NumberFormatException e) {
                        record.addElement(new Float(0.0));
                    }
                    break;
                default:
                    if (DEBUG) logger.info(DBC + "Oh - don't know how to parse " + fielddef[i].fieldtype);
            }
        }
        return record;
    }
    ...
    public Integer[] getIntegerCol(int col, int start, int end) throws java.io.IOException, DbfFileException {
        ...
        if (fielddef[col].fieldtype != 'N') throw new DbfFileException(DBC + "Column " + col + " is not Integer " + fielddef[col].fieldtype);
        ...
        try {
            for (i = start; i < end; i++) {
                ...
                sb = GetDbfRec(i);
                record = sb.toString();
                if (DEBUG) logger.info(DBC + record.substring(fielddef[col].fieldstart, fielddef[col].fieldstart + fielddef[col].fieldlen).trim() + "*");
    ...
    public Float[] getFloatCol(int col, int start, int end) throws DbfFileException, java.io.IOException {
        ...
        if (fielddef[col].fieldtype != 'F' && fielddef[col].fieldtype != 'N') throw new DbfFileException("Dbf->Column " + col + " is not Float " + fielddef[col].fieldtype);
