...
public class ExampleRunner extends JFrame {
    ...
    private static final String HEADER = "<html><body><h2 align=\"center\">Ardor3d Examples</h2>" + "<p>Choose an example in the tree and select Run. Movement keys are: W, A, S, and D. Control view with left/right buttons.</p>" + "<p align=\"center\"><img src=\"" + ResourceLocatorTool.getClassPathResource(ExampleRunner.class, "com/ardor3d/example/media/images/ardor3d_white_256.jpg") + "\"></p></body></html>";
    ...
    public ExampleRunner() {
        ...
        browseSelectedAction = new AbstractAction() {
...

            {
                ...
                putValue(Action.SHORT_DESCRIPTION, "View the source code. (Requires active Internet connection.) Ctrl-B");
        ...
        for (final Action action : new Action[] { firstMatchAction, nextMatchAction, previousMatchAction, runSelectedAction, browseSelectedAction }) {
            toolbar.getActionMap().put(action.getValue(Action.NAME), action);
            toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) action.getValue(Action.ACCELERATOR_KEY), action.getValue(Action.NAME));
        }
    ...
    private void updateDescription() {
        ...
        if (selectedClass == null) {
            ...
            if (purpose != null) {
                try {
                    final URL imageURL = ResourceLocatorTool.getClassPathResource(ExampleRunner.class, purpose.thumbnailPath());
                    if (imageURL != null) {
            ...
            if ("".equals(imgURL)) {
                imgURL = "<img src=\"" + ResourceLocatorTool.getClassPathResource(ExampleRunner.class, "com/ardor3d/example/media/images/ardor3d_white_256.jpg") + "\"/>";
            }
            lDescription.setText("<html><body><h2 align=\"center\">" + selectedClass.getSimpleName() + "</h2>" + "<p>" + htmlDescription + "</p>" + "<p align=\"center\">" + imgURL + "</p></body></html>");
        }
    }
    ...
    private String getHtmlDescription(final Purpose purpose) {
        ...
        try {
            final ResourceBundle bundle = ResourceBundle.getBundle(purpose.localisationBaseFile(), Locale.getDefault(), ClassLoader.getSystemClassLoader());
    ...
    private static ImageIcon getIcon(final String name) {
        return new ImageIcon(ResourceLocatorTool.getClassPathResource(ExampleRunner.class, "com/ardor3d/example/media/icons/" + name));
    }
    ...
    class ClassTreeModel implements TreeModel, SearchFilter {
        ...
        public int updateMatches(final String pattern) {
            ...
            for (final Entry<Class<?>, Boolean> entry : classMatches.entrySet()) {
                ...
                if (matches) {
                    final TreePath path = new TreePath(new Object[] { root, entry.getKey().getPackage(), entry.getKey() });
            ...
            if (classFileFilter == null) {
                classFileFilter = new FileFilter() {
                    ...
                    public boolean accept(final File pathname) {
                        return (pathname.isDirectory() && !pathname.getName().startsWith(".")) || (pathname.getName().endsWith(".class") && pathname.getName().indexOf('$') < 0);
                    }
                };
            }
        ...
        protected void fireTreeNodeChanged(final TreePath tp) {
            ...
            final TreeModelEvent ev = new TreeModelEvent(this, parentPath, new int[] { getIndexOfChild(parentObj, obj) }, new Object[] { obj });
            for (final TreeModelListener l : listeners.getListeners(TreeModelListener.class)) {
        ...
        protected void fireTreeNodeChanged(final Package pkg) {
            final TreeModelEvent ev = new TreeModelEvent(this, new Object[] { root }, new int[] { getIndexOfChild(root, pkg) }, new Object[] { pkg });
            for (final TreeModelListener l : listeners.getListeners(TreeModelListener.class)) {
                l.treeNodesChanged(ev);
            }
        }
        ...
        protected void fireTreeNodeChanged(final Class<?> clazz) {
            ...
            final TreeModelEvent ev = new TreeModelEvent(this, new Object[] { root, pkg }, new int[] { getIndexOfChild(pkg, clazz) }, new Object[] { clazz });
            for (final TreeModelListener l : listeners.getListeners(TreeModelListener.class)) {
                l.treeNodesChanged(ev);
            }
        }
