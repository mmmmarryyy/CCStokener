...
class GoogleAppEngineHttpProxyEventService implements HttpProxyEventService, RpcCallback<HttpResponseExchange> {
    ...
    GoogleAppEngineHttpProxyEventService(FetchServiceSelector selector, Executor workerExecutor) {
    ...
    protected boolean isProxyRequestReady() {
        ...
        if (contentLength > 0 && forwardRequest.getBody().length < contentLength) {
            return false;
        }
        return true;
    }
    ...
    protected HttpRequestExchange buildForwardRequest(HttpRequest recvReq) {
        ...
        if (!containRangeHeader && Config.getInstance().isInjectRangeHeaderSitesMatchHost(recvReq.getHeader(HttpHeaders.Names.HOST))) {
            ...
            gaeRequest.setHeader(HttpHeaders.Names.RANGE, new RangeHeaderValue(0, fetchSizeLimit - 1));
        } else {
            if (logger.isDebugEnabled()) {
    ...
    public void callBack(RpcCallbackResult<HttpResponseExchange> result) {
        try {
            ...
            int fetchSizeLimit = Config.getInstance().getFetchLimitSize();
            if (forwardResponse.isResponseTooLarge() || forwardResponse.getResponseCode() == 400) {
                if (logger.isDebugEnabled()) {
            ...
            if (channel.isConnected()) {
                ...
                if (null != contentRangeValue) {
                    ...
                    if (!originalRequest.containsHeader(HttpHeaders.Names.RANGE)) {
                        ...
                        forwardResponse.setHeader(HttpHeaders.Names.CONTENT_LENGTH, (returnContentRange.getLastBytePos() - returnContentRange.getFirstBytePos() + 1) + "");
                        if (logger.isDebugEnabled()) {
                            logger.debug("Range get response content-range header is " + returnContentRange);
                        }
                    }
                }
                ...
                if (forwardResponse.getResponseCode() >= 400) {
                    if (callback != null) {
                        callback.onProxyEventFailed(this, response, originalProxyEvent);
                        return;
                    }
                }
                ChannelFuture future = channel.write(response);
                if (null != contentRange && contentRange.getLastBytePos() < (contentRange.getInstanceLength() - 1)) {
                    chunkedInput = new RangeHttpProxyChunkedInput(selector, workerExecutor, forwardRequest, contentRange.getLastBytePos() + 1, contentRange.getInstanceLength());
