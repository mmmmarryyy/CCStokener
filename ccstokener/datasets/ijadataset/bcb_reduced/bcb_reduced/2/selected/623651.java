 ...
 * [<label for="lil.institution.name" title="Rat der Europ�ischen Union">Council</label>]
 * </td>
 * <td colspan="2" class="lilRecordRigthFooter" align="right">
 * <a href="SearchByQuery.do?method=searchDetail&lilId=931334&langId=&query=Landwirtschaft&sourceLanguage=de&domain=0&matching=&start=0&next=1&amp;targetLanguages=en" title="F�r den Zugang zum vollst�ndigen Eintrag hier klicken">
 * <b>Vollst�ndiger Eintrag</b></a></td></tr>
 * <tr height="20" onMouseover="changeColor(this,true)" onMouseOut="changeColor(this,false)" title="F�r Details zur Sprachebene hier klicken" onclick="window.location.href='FindTermsByLilId.do?lilId=931334&langId=de'">
 * <div class="termRecord"><b>DE</b></div>
 * <td width="100%" class="lilRecord"><div class="termRecord">CSSA</div></td>
 * <td width="100%" class="lilRecord"><div class="termRecord">Ausschuss f�r den Arbeitsschutz in der <span style="color:blue;background:#CCE698">Landwirtschaft</span></div></td>
...
public class IateTerminology extends BasicDataSource {
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 1) sourceLanguage = args[1];
        if (args.length > 2) targetLanguage = args[2];
        ...
        } else System.out.println("Results found= " + 0);
    }
    ...
    private Vector<MultiLingualObject> parseResult(String queryresult) {
        ...
        while (Matcher.find()) {
            ...
            if (iMonos > 0) allTermTables.add(multi);
        }
    ...
    public Element translate(Element transUnit, Element file, XliffDocument xliffDocument, String sourceLanguage, String targetLanguage, int matchSimilarity, Hashtable<String, Object> translationParameters) throws OpenTMSException {
        try {
            ...
            if (translationParameters != null) translationParameters.put("termMatch", "termMatch");
            for (int k = 0; k < sourceTerms.length; k++) {
                Vector<MultiLingualObject> multiresults = null;
                if (dictionaryTable.containsKey(sourceTerms[k])) multiresults = dictionaryTable.get(sourceTerms[k]); else {
                ...
                if ((multiresults != null)) {
                    for (int i = 0; i < multiresults.size(); i++) {
                        ...
                        for (int j = 0; j < sourcemonos.size(); j++) {
                            if (sourcemonos.get(j).getPlainTextSegment().indexOf(sourceTerms[k]) == -1) continue;
                            Element alttrans = xliffDocument.addAltTrans(transUnit, sourcemonos.get(j), targetmonos, (int) 0, translationParameters);
                            if (alttrans != null) alttrans.setAttribute("origin", this.getDataSourceType());
                            if (alttrans != null) alttrans.setAttribute("match-quality", "IATE");
                        }
                    }
                }
            }
