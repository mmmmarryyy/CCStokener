    ...
    public static synchronized String lang(final String key) {
        if (key == null) {
            if (CHECK && check.size() != 0) {
                ...
                while (it.hasNext()) Main.errln("%." + SUFFIX + ": '%' not used", Prop.language, it.next());
            }
            return null;
        }
        final String val = texts.get(key);
        if (val == null) {
            if (texts.size() != 0) Main.errln("%." + SUFFIX + ": '%' missing", Prop.language, key);
            return "?????";
        }
