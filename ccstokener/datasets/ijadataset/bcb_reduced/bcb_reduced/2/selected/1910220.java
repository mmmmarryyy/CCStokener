    ...
    */
    ...
    private ClassLoader getNestedClassLoader(Class type, ClassLoader root) {
        ...
        while (!(root instanceof URLClassLoader)) {
            ...
            root = (ClassLoader) AccessController.doPrivileged(new PrivilegedAction() {
    ...
    private WSDL4JWrapper(URL wsdlURL, Definition wsdlDefinition, ConfigurationContext configContext, JAXWSCatalogManager catalogManager, boolean limitMemory, int memoryType) throws WSDLException {
        ...
        if ((wsdlDefinition != null) && !(wsdlDefinition instanceof WSDLDefinitionWrapper)) {
            if (configContext != null && configContext.getAxisConfiguration() != null) {
                ...
                this.wsdlDefinition = new WSDLDefinitionWrapper(wsdlDefinition, wsdlURL, limitMemory, 2);
            }
        } else {
    ...
    public Definition loadDefinition() {
        ...
        if (wsdlExplicitURL != null) {
            try {
                ...
                if (urlConn != null) {
                    try {
                        ...
                        if (is != null) {
                            ...
                            final CatalogWSDLLocator locator = new CatalogWSDLLocator(wsdlExplicitURL, is, getThreadClassLoader(), catalogManager);
                            if (log.isDebugEnabled()) {
                                log.debug("Loading WSDL using ModuleWSDLLocator from base " + "location: " + wsdlExplicitURL);
                            }
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug("Using ModuleWSDLLocator was not successful for loading " + "WSDL due to the following error: " + e.toString() + ". The " + "WSDL will be read from the WSDL location: " + wsdlExplicitURL);
                        }
                    }
                }
                if (def == null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("An error occurred while attempting to load the WSDL " + "file at the following location: " + wsdlExplicitURL);
                }
