...
public class TestManyDisseminations extends FedoraServerTestCase {
    ...
    public void testManySaxonDisseminationsX() throws Exception {
        doDissemTest(DATA_OBJECT_PID + "/" + SDEF_OBJECT_PID + "/getIDFrom" + X_DS, false);
    }
    ...
    public void testManySaxonDisseminationsE() throws Exception {
        doDissemTest(DATA_OBJECT_PID + "/" + SDEF_OBJECT_PID + "/getIDFrom" + E_DS, false);
    }
    ...
    public void testManySaxonDisseminationsR() throws Exception {
        doDissemTest(DATA_OBJECT_PID + "/" + SDEF_OBJECT_PID + "/getIDFrom" + R_DS, false);
    }
    ...
    public static class ManyDisseminationsTestSetup extends TestSetup {
        ...
        private static byte[] getCModelObject() throws Exception {
            ...
            appendRel(rdf, MODEL.HAS_MODEL.localName, PID.getInstance(Models.CONTENT_MODEL_3_0.uri).toString());
            appendRel(rdf, MODEL.HAS_SERVICE.localName, SDEF_OBJECT_PID);
            closeRDF(rdf);
            appendInlineDatastream(buf, "RELS-EXT", "application/rdf+xml", Constants.RELS_EXT1_0.uri, rdf.toString());
            closeFOXML(buf);
        ...
        private static byte[] getSDefObject() throws Exception {
            ...
            appendRel(rdf, MODEL.HAS_MODEL.localName, PID.getInstance(Models.SERVICE_DEFINITION_3_0.uri).toString());
            closeRDF(rdf);
            appendInlineDatastream(buf, "RELS-EXT", "application/rdf+xml", Constants.RELS_EXT1_0.uri, rdf.toString());
            appendInlineDatastream(buf, "METHODMAP", "text/xml", Constants.SDEF_METHOD_MAP1_0.uri, getSDefMethodMap());
            closeFOXML(buf);
            return buf.toString().getBytes("UTF-8");
        }
        ...
        private static byte[] getSDepObject() throws Exception {
            ...
            appendInlineDatastream(buf, "METHODMAP", "text/xml", Constants.SDEP_METHOD_MAP1_1.uri, getSDepMethodMap());
            appendInlineDatastream(buf, "DSINPUTSPEC", "text/xml", Constants.DS_INPUT_SPEC1_1.uri, getDSInputSpec());
            appendInlineDatastream(buf, "WSDL", "text/xml", Constants.WSDL.uri, getWSDL());
            StringBuilder rdf = new StringBuilder();
            openRDF(rdf, SDEP_OBJECT_PID);
            appendRel(rdf, MODEL.HAS_MODEL.localName, PID.getInstance(Models.SERVICE_DEPLOYMENT_3_0.uri).toString());
        ...
        private static void openRDF(StringBuilder buf, String sPID) {
            buf.append("<rdf:RDF xmlns:rdf=\"http://www.w3.org/1999/02/22-" + "rdf-syntax-ns#\" xmlns:fedora-model=\"info:fedora/" + "fedora-system:def/model#\">\n  <rdf:Description " + "rdf:about=\"info:fedora/" + sPID + "\">\n");
        }
        ...
        private static byte[] getDataObject() throws Exception {
            ...
            final String url = "http://" + addr.getHostAddress() + ":" + FedoraServerTestCase.getPort() + "/" + FedoraServerTestCase.getFedoraAppServerContext() + "/get/" + DATA_OBJECT_PID + "/" + X_DS;
            appendInlineDatastream(buf, X_DS, "text/xml", Constants.OAI_DC2_0.uri, getDC());
            ...
            appendRel(rdf, MODEL.HAS_MODEL.localName, PID.getInstance(Models.FEDORA_OBJECT_3_0.uri).toString());
        ...
        private static String getXSLT() {
            StringBuilder buf = new StringBuilder();
            buf.append("<xsl:stylesheet version=\"1.0\" xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\">" + CR + "  <xsl:output encoding=\"UTF-8\" indent=\"yes\" method=\"xml\"/>" + CR + "  <xsl:template match=\"dc:identifier\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\">" + CR);
        ...
        private static void appendInlineDatastream(StringBuilder buf, String dsID, String mimeType, String formatURI, String xml) {
        ...
        private static void appendRemoteDatastream(StringBuilder buf, String dsID, String controlGroup, String mimeType, String location) {
            openDatastream(buf, dsID, controlGroup);
            openDatastreamVersion(buf, dsID + ".0", mimeType, null);
            buf.append("      <foxml:contentLocation REF=\"" + location + "\" TYPE=\"URL\"/>" + CR);
            closeDatastreamVersion(buf);
            closeDatastream(buf);
        }

        private static void openDatastream(StringBuilder buf, String dsID, String controlGroup) {
        ...
        private static void openDatastreamVersion(StringBuilder buf, String versionID, String mimeType, String formatURI) {
