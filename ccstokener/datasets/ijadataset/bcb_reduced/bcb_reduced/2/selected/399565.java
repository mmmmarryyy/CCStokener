...
public class CriticalErrorAppenderComponent extends FileAppenderComponent {
    ...
    */
    public CriticalErrorAppenderComponent(SubComponent parent, String name, String logFileNamePrefix, String logFileNameSuffix, String basePath) {
        ...
        writeLocalErrorFiles.setDescription("Flag indicating if local error files should be generated as a backup to remote logging (if remote logging is successful, " + "no file will be generated event if this property is true).");
        ...
        super.removeProperty(super.logLevelThreshold);
        dateFormat = (SimpleDateFormat) DateFormat.getDateTimeInstance();
        dateFormat.applyPattern("yyyy-MM-dd HH:mm:ss");
        super.setLayout(new ExtendedPatternLayout(MESSAGE_LAYOUT_PATTERN));
        this.serviceListenerSupport = new ServiceListenerSupport(this, "remoteLoggingServiceNames", "The names of the remote critical error log manager services to connect to. " + "Addresses for the service names will be fetched from an SNS.");
    }
    ...
    */
    public void append(final LoggingEvent event) throws Exception {
        if (remoteLoggingEnabled.booleanValue()) {
            if (!sendLogToRemoteServer(event)) {
