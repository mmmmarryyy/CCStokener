...
public class HttpURLConnectionTest extends TestCase {
    ...
    static class MockHTTPServer extends MockServer {
        ...
        public MockHTTPServer(String name, int persUses, int responseCode) throws IOException {
            super(name);
            ...
            try {
                ...
                try {
                    ...
                    for (int i = 0; i < persUses; i++) {
                        if (DEBUG) {
                            System.out.println("*** Using connection for " + (i + 1) + " time ***");
                        }
                        ...
                        if (message.startsWith("POST")) {
                            if (DEBUG) {
                                System.out.println("---- Server read client's data: ----");
                            }
                            num = is.read(buff);
                        ...
                        client.getOutputStream().write(("HTTP/1.1 " + responseCode + " OK\n" + "Content-type: text/html\n" + "Content-length: " + response.length() + "\n\n" + response).getBytes("UTF-8"));
            ...
            public void handle(String target, HttpServletRequest request, HttpServletResponse response, int dispatch) throws IOException, ServletException {
                ...
                if (noRealm) {
                    ...
                    if (auth) {
                        ...
                        response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                        response.addHeader("WWW-Authenticate", "Basic realm=\"HelloWorld\"");
                    }
                }
            }
        }
    ...
    public void testGetResponseCode() throws Exception {
        ...
        try {
            ...
            try {
                conn = (HttpURLConnection) new URL("http://localhost:" + port + "/norealm").openConnection();
            ...
            try {
                conn = (HttpURLConnection) new URL("http://localhost:" + port + "/realm").openConnection();
                assertEquals(401, conn.getResponseCode());
                assertEquals("Basic realm=\"HelloWorld\"", conn.getHeaderField("WWW-Authenticate"));
            } finally {
                if (conn != null) {
                    try {
                        conn.disconnect();
    ...
    public void testConnectWithSpaceinURL() throws Exception {
        String jettyURLwithSpace = "http://localhost:" + jettyPort + "/servlet with space";
        HttpURLConnection httpURLConnect = (HttpURLConnection) new URL(jettyURLwithSpace).openConnection();
    ...
    public void testConnectWithSpaceinURL1() throws Exception {
        String jettyURLwithSpace = "http://localhost:" + jettyPort + "/servlet with space?arg1=value>1&arg2=%aval%1Aue&arg3=#";
        HttpURLConnection httpURLConnect = (HttpURLConnection) new URL(jettyURLwithSpace).openConnection();
        httpURLConnect.setDoOutput(true);
        httpURLConnect.connect();
        assertEquals(200, httpURLConnect.getResponseCode());
    ...
    public void testGetOutputStream() throws Exception {
        MockServer httpServer = new MockServer("ServerSocket for HttpURLConnectionTest");
        httpServer.start();
        synchronized (bound) {
        ...
        HttpURLConnection c = (HttpURLConnection) new URL("http://127.0.0.1:" + httpServer.port()).openConnection();
        c.setDoOutput(true);
    ...
    public void testConnectionPersistence() throws Exception {
        MockHTTPServer httpServer = new MockHTTPServer("HTTP Server for persistence checking", 2);
        httpServer.start();
        synchronized (bound) {
            if (!httpServer.started) {
                bound.wait(5000);
        ...
        while ((bytesRead = is.read(buffer)) > 0) {
            if (DEBUG) {
                System.out.println("Client got response: '" + new String(buffer, 0, bytesRead, "UTF-8") + "'");
            }
            totalBytes += bytesRead;
        }
        HttpURLConnection c2 = (HttpURLConnection) new URL("http://localhost:" + httpServer.port()).openConnection();
        c2.setDoInput(true);
    ...
    public void testProxiedConnectionPersistence() throws Exception {
        ...
        HttpURLConnection c = (HttpURLConnection) new URL("http://some.host:1234").openConnection(new Proxy(Proxy.Type.HTTP, new InetSocketAddress("localhost", httpServer.port())));
        if (DEBUG) {
            System.out.println("Actual connection class: " + c.getClass());
        }
        c.setDoOutput(true);
        ...
        HttpURLConnection c2 = (HttpURLConnection) new URL("http://some.host:1234").openConnection();
        c2.setDoOutput(true);
        c2.setDoInput(true);
        c2.setConnectTimeout(5000);
        c2.setReadTimeout(5000);
    ...
    public void testSecurityManager() throws MalformedURLException, IOException, InterruptedException {
        try {
            ...
            while ((bytesRead = is.read(buffer)) > 0) {
                if (DEBUG) {
                    System.out.println("Client got response: '" + new String(buffer, 0, bytesRead) + "'");
                }
                totalBytes += bytesRead;
            }
            System.setSecurityManager(sm);
