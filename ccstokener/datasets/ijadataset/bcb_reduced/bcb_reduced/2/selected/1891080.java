...
public class HarvesterServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        try {
            ...
            throw new ServletException("Could not initialize due to " + exception.getMessage());
        }
        newGenXMLGenerator = servlet.util.NewGenXMLGenerator.getInstance();
        homeFactory = servlet.util.HomeFactory.getInstance();
        try {
            home = (ejb.bprocess.OAIPMH.AutoHarvesterSessionHome) ejb.bprocess.util.HomeFactory.getInstance().getRemoteHome("AutoHarvesterSession");
        } catch (Exception exp) {
            exp.printStackTrace();
        }
    }
