...
public class SampleHTTPManager extends LogFileManager {
    ...
    private static final String URL_PATTERN = PROTOCOL + "://{" + URL_PARAM_HOST + "}[:{" + URL_PARAM_PORT + "}][/{" + URL_PARAM_FILE + "}]";
    ...
    public LogURL getTemplateLogURL() {
        if (logURL == null) {
            ...
            logURL.addParameter(new LogURLParameter(URL_PARAM_PORT, LogURLParameterType.INTEGER, DEFAULT_PORT, true));
            logURL.addParameter(new LogURLParameter(URL_PARAM_FILE, LogURLParameterType.FILEPATH, null, true));
        }
        return logURL;
    }
    ...
    public ManagerSpecVersion getSpecificationVersion() {
        return ManagerSpecVersion.V1_1;
    }
