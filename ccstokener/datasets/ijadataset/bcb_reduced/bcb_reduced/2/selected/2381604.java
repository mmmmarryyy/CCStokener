...
public class FetchFromGEODialog extends Dialog {
    ...
    public void createContents() {
        ...
        ImageDescriptor id = ImageDescriptor.createFromFile(FetchFromGEODialog.class, "/org/gvt/icon/cbe-icon.png");
        ...
        fetchButton.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent arg0) {
                if (main.getRootGraph() == null) {
                    MessageDialog.openError(main.getShell(), "Error!", "No BioPAX model loaded.");
                    return;
                }
                if (seriesIDText.getText().startsWith("GSE")) {
                    selectedSeries = seriesIDText.getText();
                } else {
                    MessageDialog.openWarning(main.getShell(), "Invalid input!", "Enter a valid series ID!");
                    return;
                }
                shell.close();
