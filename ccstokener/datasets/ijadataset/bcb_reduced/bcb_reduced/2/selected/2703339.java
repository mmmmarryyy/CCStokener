...
public class WsmanConnection {
   ...
   */
    ...
    protected String stampRequest(Document document) {
        ...
        Element headerElt = WsmanUtils.findChild(document.getDocumentElement(), WsmanUtils.SOAP_NAMESPACE, "Header");
        Element msgElt = WsmanUtils.findChild(headerElt, WsmanUtils.ADDRESSING_NAMESPACE, "MessageID");
        if (msgElt != null) {
            StringBuffer buffer = new StringBuffer();
            buffer.append("uuid:");
            buffer.append(java.util.UUID.randomUUID().toString().toUpperCase());
            messageId = buffer.toString();
            msgElt.setTextContent(messageId);
        }
        Element toElt = WsmanUtils.findChild(headerElt, WsmanUtils.ADDRESSING_NAMESPACE, "To");
        if (toElt != null) toElt.setTextContent((String) properties.get("Address"));
        return messageId;
    }
        ...
        try {
            ...
            switch(conn.getResponseCode()) {
                case HttpURLConnection.HTTP_UNAUTHORIZED:
                    properties.put("Authorization", conn.getHeaderField("WWW-Authenticate"));
                    break;
    ...
    public ManagedInstance newInstance(String resourceURI) {
        return new ManagedInstance(this, getXmlLoader().createDocument(resourceURI));
    }
    ...
    public ManagedReference newReference(String resourceURI) {
        ...
        try {
            ref = new ManagedReference(this, getXmlLoader().loadDocument("EprXMLDoc.xml"));
