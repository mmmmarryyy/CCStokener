package com.foosball3d.sandbox;
...
public class TestChooser extends JDialog {
    ...
    private Class load(String name) {
        if (name.endsWith(".class") && (name.indexOf("Test") >= 0 || name.indexOf("Lesson") >= 0) && name.indexOf('$') < 0) {
    ...
    private FileFilter getFileFilter() {
        return new FileFilter() {
            ...
            public boolean accept(File pathname) {
                return (pathname.isDirectory() && !pathname.getName().startsWith(".")) || (pathname.getName().endsWith(".class") && (pathname.getName().indexOf("Test") >= 0 || pathname.getName().indexOf("Lesson") >= 0) && pathname.getName().indexOf('$') < 0);
            }
        };
    }
    ...
    protected void addDisplayedClasses(Vector<Class> classes) {
        try {
            ...
            logger.logp(Level.SEVERE, this.getClass().toString(), "addDisplayedClasses(classes)", "Exception", e);
        }
