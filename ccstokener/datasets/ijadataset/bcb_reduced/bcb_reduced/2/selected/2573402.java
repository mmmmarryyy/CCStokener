...
public class AxisAndAlliesDotOrgPBEMMessenger implements IPBEMTurnSummaryMessenger, IPBEMSaveGameMessenger {
    ...
    private void readCookies(URLConnection urlConn) {
        ...
        for (boolean done = false; !done; n++) {
            ...
            if (headerKey != null || headerVal != null) {
                if (!headerKey.equals("Set-Cookie")) continue;
                String cookie = headerVal.substring(0, headerVal.indexOf(";"));
                if (m_cookies == null) {
                    m_cookies = cookie;
                    continue;
                }
                if (m_cookies.indexOf(cookie) == -1) m_cookies = m_cookies + "; " + cookie;
            } else {
                done = true;
            }
        }
    ...
    public boolean postTurnSummary(String summary, String screenshotRef, String saveGameRef) {
        ...
        if (m_saveGameFileIn == null) m_saveGameRef = saveGameRef;
        if (m_cookies == null && !postLogin()) return false;
        if (!goToForum()) return false;
        if (!preparePost()) return false;
        try {
        ...
        if (m_screenshotRef != null) screenshotSummary = "\nScreenshot: " + m_screenshotRef;
        if (m_saveGameRef != null) saveGameSummary = "\nSave Game : " + m_saveGameRef;
        ...
        try {
            ...
            if (code == 200) {
                ...
                p3 = Pattern.compile(".*?session timed out.*");
                while ((line = in.readLine()) != null) {
