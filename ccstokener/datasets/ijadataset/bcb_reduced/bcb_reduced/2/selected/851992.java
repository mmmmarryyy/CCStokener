...
public class Scrobbler {
    ...
    public ResponseStatus handshake(String password) throws IOException {
        ...
        String url = String.format("%s?hs=true&p=1.2.1&c=%s&v=%s&u=%s&t=%s&a=%s", handshakeUrl, clientId, clientVersion, user, time, auth);
    ...
    public ResponseStatus handshake(Session session) throws IOException {
        ...
        String url = String.format("%s?hs=true&p=1.2.1&c=%s&v=%s&u=%s&t=%s&a=%s&api_key=%s&sk=%s", handshakeUrl, clientId, clientVersion, user, time, auth, session.getApiKey(), session.getKey());
        return performHandshake(url);
    }
