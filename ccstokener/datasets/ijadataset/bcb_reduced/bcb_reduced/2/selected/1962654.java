...
public class LinkedOpenData {
    ...
    private String builQuery(String query, String type) {
        ...
        if (type.equals(SUBCLASS_QUERY)) {
            builtQuery.append("SELECT distinct ?s  ?o" + "       WHERE { " + "       <http://dbpedia.org/ontology/" + query + "> rdfs:subClassOf ?o  ." + " } ");
    ...
    public HashSet<String> getRelatedTerms(String individual) {
        ...
        String queryString = " PREFIX owl: <http://www.w3.org/2002/07/owl#> " + " 	PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>" + " 	PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>" + " 	PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>" + " 	PREFIX foaf: <http://xmlns.com/foaf/0.1/>" + " 	PREFIX dc: <http://purl.org/dc/elements/1.1/>" + " 	PREFIX : <http://dbpedia.org/resource/>" + " 	PREFIX dbpedia2: <http://dbpedia.org/property/>" + " 	PREFIX dbpedia: <http://dbpedia.org/>" + " 	PREFIX skos: <http://www.w3.org/2004/02/skos/core#>		" + " SELECT ?z ?p ?c ?m ?o " + " WHERE { " + "   ?rs owl:unionOf ?z . " + " ?z ?m ?o." + " (?o ?p ?c)." + "}";
    ...
    public void findFriends() {
        ...
        Query sparql = QueryFactory.create("SELECT ?aname ?bname " + "WHERE { " + "?a <http://xmlns.com/foaf/0.1/knows> ?b . " + "?a <http://xmlns.com/foaf/0.1/name> ?aname . " + "?b <http://xmlns.com/foaf/0.1/name> ?bname . " + "}");
