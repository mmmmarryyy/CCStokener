...
public class RefCheck {
    ...
    *
	 ...
	 * @since           1.0
	**/
    public void check(String path) throws Exception {
        if (mListOnly) mVerbose = true;
        if (path == null) return;
        ...
        if (filePath.isDirectory()) {
            list = filePath.listFiles(new FileFilter() {
                ...
                public boolean accept(File pathname) {
                    return pathname.getName().endsWith(mExtension);
                }
            });
        } else {
            list[0] = filePath;
	...
	**/
    public void checkFile(String path) {
        try {
            ...
            if (ok) {
                if (!mListOnly) System.out.println("[OK]");
            } else {
                if (!mVerbose) System.out.print(path + " ");
    ...
    *
	 ...
	 * @since           1.0
	**/
    public void loadResourceIndex(String path) throws Exception {
        File filePath = new File(path);
        File[] list = new File[1];
        if (filePath.isDirectory()) {
            list = filePath.listFiles(new FileFilter() {
	...
	**/
    public boolean lookupID(String id) {
        ...
        if (igpp.util.Text.isInList(id, mLocalList)) {
            if (mVerbose) System.out.println("Found in local list.");
            return true;
        }
        try {
            Document doc = XMLGrep.parse(mRegistry + "?c=yes&id=" + id);
            ArrayList<Pair> docIndex = XMLGrep.makeIndex(doc, "");
