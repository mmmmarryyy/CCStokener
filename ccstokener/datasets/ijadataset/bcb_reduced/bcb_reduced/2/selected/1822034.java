...
public class EnhancementTable {
    ...
    public EnhancementTable(String source) throws McIDASException {
        try {
            dataStream = new DataInputStream(new BufferedInputStream(new FileInputStream(source)));
        } catch (Exception e) {
            try {
                dataStream = new DataInputStream(new BufferedInputStream(new URL(source).openStream()));
            } catch (Exception e2) {
    ...
    private void readRGBValues() throws McIDASException {
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 256; j++) rgbValues[i][j] = dataStream.readInt();
            }
        } catch (Exception e) {
    ...
    public int getIndex(int red, int green, int blue) {
        ...
        for (inx = 0; inx < 256; inx++) {
            if (rgbValues[0][inx] == red && rgbValues[1][inx] == green && rgbValues[2][inx] == blue) {
                return inx;
            }
        }
        return -1;
    ...
    public String toString() {
        ...
        sb.append("\n");
        for (int i = 0; i < 256; i++) {
            sb.append(" " + i + "  " + rgbValues[0][i] + " " + rgbValues[1][i] + " " + rgbValues[2][i]);
            sb.append("\n");
        }
        return sb.toString();
    }
