...
public class BrutosContext extends AbstractWebApplicationContext implements ConfigurableWebApplicationContext {
    ...
    private void loadContext(ServletContextEvent sce) {
        ...
        try {
            ...
            if (logger != null) logger.info(String.format("Initialization processed in %d ms", (System.currentTimeMillis() - time)));
        }
    }
    ...
    private void registerDefaultScopes(ServletContextEvent sce) {
        ...
        if (iocManager.getProvider().containsBeanDefinition("customScopes")) {
            CustomScopeConfigurer customScopes = (CustomScopeConfigurer) iocManager.getInstance("customScopes");
            Map scopes = customScopes.getCustomScopes();
            Set i = scopes.keySet();
            for (Object key : i) getScopes().register((String) key, (Scope) scopes.get(key));
        }
        logger.info("Scopes:");
        Map scopes = getScopes().getScopes();
        Set i = scopes.keySet();
        for (Object key : i) logger.info(String.valueOf(key));
    }

    private void registerCustomEditors() {
        if (iocManager.getProvider().containsBeanDefinition("customEditors")) {
            CustomEditorConfigurer customEditors = (CustomEditorConfigurer) iocManager.getInstance("customEditors");
            Map editors = customEditors.getCustomEditors();
            Set i = editors.keySet();
            for (Object key : i) EditorConfigurer.registerPropertyEditor((String) key, (PropertyEditor) editors.get(key));
        }
    }
    ...
    private void loadLogger(ServletContext sc) {
        ...
        sc.setAttribute(BrutosConstants.LOGGER, this.loggerProvider);
    ...
    private void loadManager(Configuration config, ServletContextEvent sce) {
        this.iocManager = new IOCManager(IOCProvider.getProvider(config));
        ...
        sc.setAttribute(BrutosConstants.IOC_MANAGER, this.iocManager);
        sc.setAttribute(BrutosConstants.IOC_PROVIDER, this.iocManager.getProvider());
        sc.setAttribute(BrutosConstants.WEBFRAME_MANAGER, this.webFrameManager);
        sc.setAttribute(BrutosConstants.INTERCEPTOR_MANAGER, this.interceptorManager);
        sc.setAttribute(BrutosConstants.VIEW_PROVIDER, this.viewProvider);
        sc.setAttribute(BrutosConstants.VALIDATOR_PROVIDER, this.viewProvider);
    }
    ...
    private void loadServices() throws IOException {
        ...
        while (urlServices.hasMoreElements()) {
            ...
            if (serviceClass != null) {
                ...
                for (String s : ss) {
                    if (!serviceNames.contains(s)) serviceNames.add(s);
                }
            }
        }
    ...
    private List<String> getCustomServices() {
        ...
        String stringService = configuration.getProperty("org.brandao.brutos.applicationcontext");
        if (stringService != null) {
            String[] ss = stringService.replaceAll(" ", "").split("\\,");
            for (String s : ss) {
                if (!serv.contains(s)) serv.add(s);
            }
        }
        return serv;
    }
    ...
    private void resolveController(ServletContext context) {
        ...
        context.setAttribute(BrutosConstants.CONTROLLER_RESOLVER, instance);
    }

    private void methodResolver(ServletContext context) {
        try {
            MethodResolver instance = (MethodResolver) Class.forName(configuration.getProperty("org.brandao.brutos.controller.method_resolver", "org.brandao.brutos.DefaultMethodResolver"), true, Thread.currentThread().getContextClassLoader()).newInstance();
            context.setAttribute(BrutosConstants.METHOD_RESOLVER, instance);
        } catch (Exception e) {
    ...
    public synchronized void stop(ServletContextEvent sce) {
        ...
        if (this.iocManager != null && this.iocManager.getProvider() != null) this.iocManager.getProvider().destroy();
        if (this.loggerProvider != null) this.loggerProvider.destroy();
    ...
    public static BrutosContext getCurrentInstance() {
        BrutosContext brutosCore = (BrutosContext) ContextLoaderListener.currentContext.getAttribute(BrutosConstants.ROOT_APPLICATION_CONTEXT_ATTRIBUTE);
        if (brutosCore == null) {
        ...
        Throwable ex = (Throwable) ContextLoaderListener.currentContext.getAttribute(BrutosConstants.EXCEPTION);
        if (ex != null) throw new BrutosException(ex);
        return brutosCore;
    }
    ...
    public Controller getController() {
        ...
        return (Controller) brutosInstance.getRequest().getAttribute(BrutosConstants.CONTROLLER);
    }

    public MethodResolver getMethodResolver() {
        BrutosContext brutosInstance = BrutosContext.getCurrentInstance();
        return (MethodResolver) brutosInstance.getContext().getAttribute(BrutosConstants.METHOD_RESOLVER);
    }

    public InterceptorManager getInterceptorManager() {
        BrutosContext brutosInstance = BrutosContext.getCurrentInstance();
        return (InterceptorManager) brutosInstance.getContext().getAttribute(BrutosConstants.INTERCEPTOR_MANAGER);
    }

    public ControllerResolver getResolveController() {
        BrutosContext brutosInstance = BrutosContext.getCurrentInstance();
        return (ControllerResolver) brutosInstance.getContext().getAttribute(BrutosConstants.CONTROLLER_RESOLVER);
    }
    ...
    private void loadServices(List<String> data) {
        for (String provider : data) {
            try {
                ...
                services.add((AbstractApplicationContext) Class.forName(provider, true, Thread.currentThread().getContextClassLoader()).newInstance());
            } catch (Exception e) {
    ...
    public LoggerProvider getLoggerProvider() {
        if (loggerProvider == null) throw new BrutosException("Logger provider was not configured"); else return loggerProvider;
    }
