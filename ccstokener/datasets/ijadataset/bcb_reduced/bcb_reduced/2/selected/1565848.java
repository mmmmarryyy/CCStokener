...
final class ClientHttpRequest extends Observable {
   ...
   */
    ...
    public long getFilePostSize(String name, File file) {
        ...
        String type = URLConnection.guessContentTypeFromName(filename);
        if (type == null) type = "application/octet-stream";
        return boundaryNumBytes() + writeNameNumBytes(name) + "; filename=\"".length() + filename.getBytes().length + 1 + newlineNumBytes() + "Content-Type: ".length() + type.length() + newlineNumBytes() + newlineNumBytes() + file.length() + newlineNumBytes();
    }
