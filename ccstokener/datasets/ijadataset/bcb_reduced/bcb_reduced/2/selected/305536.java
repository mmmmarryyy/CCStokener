...
public class HttpClient {
    ...
    public HttpResponse post() throws TopCometSysErrorException {
        ...
        for (retriedCount = 1; retriedCount <= retry; retriedCount++) {
            ...
            try {
                ...
                try {
                    ...
                    if (HttpURLConnection.HTTP_OK == responseCode) {
                        ...
                        Log.d(TAG, "Request param is invalid,errmsg is:" + con.getHeaderField(StreamConstants.ERR_MSG_HEADER));
                        throw new TopCometSysErrorException("Server response err msg:" + con.getHeaderField(StreamConstants.ERR_MSG_HEADER));
                    } else if (HttpURLConnection.HTTP_FORBIDDEN == responseCode) {
                        Log.d(TAG, "Server is deploying,sleep " + retriedCount * conf.getHttpConnectRetryInterval() + " seconds");
                        if (retriedCount == conf.getHttpConnectRetryCount()) {
                            Log.d(TAG, "May be server occure some error,please contact top tech support");
            ...
            try {
                Log.d(TAG, "Sleeping " + conf.getHttpConnectRetryInterval() + " seconds until the next retry.");
                Thread.sleep(retriedCount * conf.getHttpConnectRetryInterval() * 1000);
    ...
    private String encodeParameters(Map<String, String> params) {
        ...
        while (it.hasNext()) {
            ...
            buf.append(encode(e.getKey())).append("=").append(encode(e.getValue()));
            j++;
        }
        return buf.toString();
    }
