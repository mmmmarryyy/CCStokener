...
public class MasterNodeService {
    ...
    protected static Logger logger = LoggerFactory.getLogger(MasterNodeService.class);
    ...
    public static Event randomRetrieveAppIds() {
        if (null != sharedItems && !sharedItems.isEmpty()) {
            ...
            ev.appids[0] = sharedItems.get(rnd.nextInt(sharedItems.size())).getAppid();
            return ev;
        }
    ...
    public static Event HandleShareEvent(ShareAppIDEvent ev) {
        return ev.operation == ShareAppIDEvent.SHARE ? shareMyAppId(ev.appid, ev.email) : unshareMyAppid(ev.appid, ev.email);
    }

    public static Event shareMyAppId(String appid, String gmail) {
        AppIdShareItem share = getSharedItem(appid);
        if (null != share) {
            return new AdminResponseEvent(null, "This AppId is already shared!", -1);
        }
        if (!verifyAppId(appid)) {
            return new AdminResponseEvent(null, "Invalid AppId or Invalid snova-gae-server for this AppId!", -1);
        }
        ...
        if (null != gmail) {
            sendMail(gmail, "Thanks for sharing AppID:" + appid + "!", "Thank you for sharing your appid!");
        }
    ...
    public static Event unshareMyAppid(String appid, String gmail) {
        AppIdShareItem share = getSharedItem(appid);
        if (null == share) {
            return new AdminResponseEvent(null, "This appid is not shared before!", -1);
        }
        if (null != share.getGmail()) {
            if (!share.getGmail().equals(gmail)) {
                return new AdminResponseEvent(null, "The input email address is not equal the share email address.", -1);
            } else {
                sendMail(gmail, "Unsharing AppID:" + appid + "!", "Your appid:" + appid + " is unshared from snova master.");
            }
        }
        ...
        if (null != sharedItems) {
            for (AppIdShareItem temp : sharedItems) {
                if (temp.appid.equals(item.appid)) {
                    ...
                    asyncdatastore.delete(KeyFactory.createKey(GAEConstants.SHARED_APPID_ENTITY_NAME, temp.appid));
                    break;
                }
            }
        }
