...
public class Curl implements Serializable {
	 ...
	 */
    protected static volatile SimpleDateFormat mFormat = new SimpleDateFormat("EEE, dd-MMM-yy kk:mm:ss z");
    ...
    public static String testFetchUrl(String toFetchStr) throws ClientProtocolException, IOException {
        Curl curlTmp = new Curl();
        HttpResponse respTmp = curlTmp.get(toFetchStr);
        System.out.println(respTmp);
	 ...
	 */
    private HttpResponse makeAuth(String toFetchStr, HttpClient httpClient, HttpUriRequest m, HttpResponse respTmp) throws IOException, ClientProtocolException {
        ...
        if (statusTmp.indexOf("200 OK") > 0) {
        ...
        } else if ("HTTP/1.1 401 Unauthorized".equals(statusTmp) || "HTTP/1.0 401 Unauthorized".equals(statusTmp)) {
            {
                ...
                if (hTmp.length == 0) bRealm = bRealm.replace("$$$$$$", "Tomcat Manager Application"); else bRealm = bRealm.replace("$$$$$$", "" + hTmp);
    ...
    public static final String CACHE_NAME = Curl.class.getName() + ":Authorization";
    ...
    public HttpResponse get(String toFetchStr) throws IOException, ClientProtocolException {
        return get(toFetchStr, new String[][] {});
    }

    public HttpResponse post(String toFetchStr, String[][] headers, Map parameterMap) throws ClientProtocolException, IOException {
	 ...
	 */
    private void validateContentType(Map parameterMap, HttpPost m) throws UnsupportedEncodingException {
    ...
    private void setupProxy(HttpClient httpClient) {
        ...
        for (String scheme : schemes) {
            ...
            if (("" + proxHostTmp + proxyPortTmp).indexOf("null") == -1) {
                org.apache.http.HttpHost proxyTmp = new org.apache.http.HttpHost(proxHostTmp, Integer.parseInt(proxyPortTmp), scheme);
    ...
    private void parseCookies(HttpUriRequest request, HttpResponse response) {
        ...
        for (Header setCookieHeader : setCookieHeaders) {
            ...
            StringTokenizer tokenizer = new StringTokenizer(strCookieHeader, ";,", true);
            cookie = null;
            String name;
            while (tokenizer.hasMoreTokens()) {
                ...
                if (null == cookie) {
                    ...
                    } else if (key.equals("domain")) cookie.setDomain(value); else if (key.equals("path")) cookie.setPath(value); else if (key.equals("paath")) cookie.setPath(value); else if (key.equals("secure")) cookie.setSecure(true); else if (key.equals("comment")) cookie.setComment(value); else if (key.equals("version")) cookie.setVersion(Integer.parseInt(value)); else if (key.equals("max-age")) {
                        Date date = new Date();
            ...
            if (0 != cookies.size()) saveCookies(cookies, request);
        }
    }
    ...
    private Map<String, List<Cookie>> getOrCreateStore() {
        ...
        if (null == mCookieJar) {
            mCookieJar = new HashMap<String, List<Cookie>>();
            try {
	 ...
	 */
    public void addCookies(HttpUriRequest request) {
        List<Cookie> list;
        URI url;
        String host;
        String path;
        ...
        if (null != mCookieJar) {
            ...
            if (null != host) {
                ...
                String keyCook = null;
                if (null != domain) keyCook = domain; else keyCook = "." + host;
            ...
            if (null != list) {
                ...
                generateCookieProperty = "$Version=\"1\"; " + generateCookieProperty;
    ...
    protected void saveCookies(List<Cookie> list, HttpUriRequest request) {
        for (Cookie cookie : list) {
            ...
            if (null == domain) domain = request.getURI().getHost();
    ...
    protected List<Cookie> mergeCookies(List<Cookie> cookies, String path, List<Cookie> list) {
        ...
        if (cookies != null) copyOfCookies.addAll(cookies);
        Date expires;
        Date now;
        if (null != cookies) {
            now = new Date();
            for (Cookie cookie : copyOfCookies) {
                ...
                if ((null != expires) && expires.before(now)) {
                    ...
                    if (null == list) list = new ArrayList<Cookie>();
                    if (list.indexOf(cookie) == -1) list.add(cookie);
                }
            }
        }
        return (list);
