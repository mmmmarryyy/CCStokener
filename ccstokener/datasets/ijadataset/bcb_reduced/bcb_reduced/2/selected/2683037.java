...
public class PayPalExpressCheckoutPaymentGatewayImpl extends AbstractPayPalPaymentGatewayImpl implements PaymentGatewayExternalForm {
    ...
    public Payment authorizeCapture(final Payment paymentIn) {
        ...
        try {
            final Map<String, String> paymentResult = doDoExpressCheckoutPayment(payment.getTransactionRequestToken(), payment.getTransactionReferenceId(), payment.getPaymentAmount(), payment.getOrderCurrency());
            payment.setPaymentProcessorResult(isSuccess(paymentResult) ? Payment.PAYMENT_STATUS_OK : Payment.PAYMENT_STATUS_FAILED);
        } catch (IOException e) {
    ...
    private Map<String, String> performHttpCall(final String method, final String nvpStr) throws IOException {
        ...
        return deformatNVP(performPayPalApiCall(stringBuilder.toString()));
    }
