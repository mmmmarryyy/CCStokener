...
public class BlindScript extends LessonAdapter {
    ...
    protected Element createContent(WebSession s) {
        ...
        try {
            ...
            if (!"".equals(person.toString())) {
                ec.addElement(new StringElement("Sorry.  Could not locate record for: " + person.toString()));
            }
    ...
    private void executeTimeTrigger(WebSession s) {
        ...
        if (cal1.getTime().after(cal2.getTime())) {
            try {
                ...
                PreparedStatement statement = WebSession.getConnection(s).prepareStatement(query, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    private static boolean compile(JavaFileObject... source) {
        ...
        final JavaCompiler.CompilationTask task = compiler.getTask(null, null, null, null, null, Arrays.asList(source));
        return task.call();
    }
