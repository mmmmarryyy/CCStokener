...
final class SWGPostLaunch {
     ...
     */
    private static final String DOCS_INTRO = " The documentation in SWGAide is most often per panel, hence," + "<br/>press <b>&nbsp;F1&nbsp;</b> while at the panel you have " + "questions about.<br/>For further information and for " + "questions, go to<br/>";
    ...
    private void deleteObsoleteFiles() {
        ...
        if (datBak.exists()) datBak.delete();
    }
    ...
    private void docsCopy() {
        try {
            ...
            makeDirPath(dPath);
            List<String> fList = ZReader.entries(new JarFile("SWGAide.jar"), jPath, ".html", ".png");
            docsCopy(fList, jPath.substring(4), dPath);
            Map<String, String> iMap = docsIndexMap(SWGAide.class.getResourceAsStream("docs/index_en.txt"));
    ...
    private void docsCopy(List<String> names, String jarPath, File targetDir) {
        for (String name : names) {
            if (name.endsWith("html") || name.endsWith("png")) {
                InputStream in = SWGAide.class.getResourceAsStream(jarPath + '/' + name);
    ...
    private String[] docsSplit(String str) {
        ...
        if (subFeature < 0) return new String[] { ZString.tac(str.substring(0, mainFeature)), null };
        int language = str.lastIndexOf('_');
        String feat = ZString.tac(str.substring(mainFeature + 1, subFeature));
        String subf = null;
    ...
    private void docsWrite(ZWriter wr, String... strings) throws Exception {
        ...
        for (String s : strings) wr.writeExc(s);
        wr.writelnExc("</font>");
    }
    ...
    private void docsWriteIndex() throws Exception {
        ...
        docsLink(wr, "http://www.swgcraft.org/forums/viewforum.php?f=43", "<nobr>SWGAide @ SWGCraft.org</nobr>");
        wr.writelnExc("<p><p><p>");
        docsWrite(wr, "Documentation is found, partial or complete,<br/>", "for the following languages:");
        wr.writeExc("<p>");
        ...
        if (langDirs == null) docsLink(wr, "docs", "ERROR: docs"); else {
            ...
            for (File d : langDirs) {
                ...
                docsLink(wr, idxFile.getPath(), ZString.tac(langDef) + " &mdash; " + ZString.tac(langEng));
                wr.writeExc("<p>");
            }
        }
        wr.eol();
     ...
     * @throws Exception if there is an error
     */
    private void docsWriteIndex(List<String> files, Map<String, String> map, File directory) throws Exception {
        ...
        for (String e : files) {
            ...
            String m = map.get(e);
            String[] split = docsSplit(e);
            wr.writeExc("<tr>");
            if (split[1] == null) {
                wr.writeExc("<td colspan=2>");
                wr.writeExc("&nbsp;<br/>");
                docsLink(wr, lang + "/" + e, m == null ? split[0] : m);
            } else {
                wr.writeExc("<td width=40>&nbsp;</td><td>");
                docsLink(wr, lang + "/" + e, m == null ? split[1] : m);
            }
            wr.writelnExc("</td></tr>");
        }
    ...
    private void existsUpdate() {
        try {
            URL url = new URL(SWGConstants.swgAideURL + SWGConstants.swgVersionTXT);
            ...
            if (update != null && !update.startsWith("null")) {
                SwingUtilities.invokeLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        String msg = String.format("SWGAide version %s%n" + "is available, visit%n%s", update, u);
                        int ret = JOptionPane.showOptionDialog(frame, msg, "Update available", JOptionPane.OK_CANCEL_OPTION, JOptionPane.INFORMATION_MESSAGE, null, opt, opt[0]);
                        if (ret == JOptionPane.OK_OPTION) ZHtml.browser(u);
                    }
                });
            }
        } catch (Exception e) {
    ...
    private boolean isSame() {
        return SWGFrame.getPrefsKeeper().getVersion().equals(SWGConstants.version);
    }
