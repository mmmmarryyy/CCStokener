...
public class GLTextureFilter implements GLConstants, PConstants {
   ...
   */
    public void apply(GLTexture[] srcTex, GLTexture[] destTex, GLModel destModel) {
        ...
        if (crop) {
            grid.render(srcWidth, srcHeight, destWidth, destHeight, srcTex.length, cropX0, cropY0, cropX1, cropY1);
        } else {
            grid.render(srcWidth, srcHeight, destWidth, destHeight, srcTex.length, 0, 0, srcWidth, srcHeight);
        }
        if (0 < srcTex.length) {
            unbindSrcTex(srcTex);
        } else {
   ...
   */
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (name.equals("intexture")) {
                ...
                if ((0 <= texUnit) && (texUnit < n)) {
                    ...
                    if (offsetName != null) srcTexOffsetNames[texUnit] = offsetName;
    ...
    protected void addParameter(XMLElement xml) {
        ...
        if (parTypeStr.equals("clock")) {
            clockDataName = parName;
        } else if (parTypeStr.equals("tint")) {
            destColorName = parName;
        } else if (parTypeStr.equals("size")) {
            destTexSizeName = parName;
        } else {
    ...
    String fixShaderFilename(String filename, String rootPath) {
        ...
        if (!rootPath.equals("") && (fixedFN.indexOf(rootPath) != 0)) fixedFN = rootPath + fixedFN;
        return fixedFN;
    }
   ...
   */
    ...
    void setupShader(GLTexture[] srcTex, int w, int h, float destR, float destG, float destB, float destA) {
        ...
        for (int i = 0; i < paramsArray.length; i++) paramsArray[i].copyToShader();
    }

    protected void checkDestTex(GLTexture[] destTex, int w, int h) {
        for (int i = 0; i < destTex.length; i++) if (!destTex[i].available()) {
            destTex[i].init(w, h);
        }
    }
}
