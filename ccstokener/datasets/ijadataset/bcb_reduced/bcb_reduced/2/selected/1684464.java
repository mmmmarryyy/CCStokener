...
public class BBSDocAction implements IDocAction {
    ...
    static List<DocObject> dealFirstPage(List<DocObject> list, int endFromIndex) {
        for (int i = list.size() - 1; endFromIndex > 0 && i >= 0; i--, endFromIndex--) list.remove(i);
        return list;
    }
    ...
    static String boardDocUrl(BoardObject board, boolean isTitle, int start) {
        String url = HttpConfig.bbsURL() + (isTitle ? HttpConfig.BBS_DOC_TITLE : HttpConfig.BBS_DOC) + board.getName() + (start == 0 ? "" : "&" + HttpConfig.BBS_DOC_START_PARAM_NAME + "=" + start);
        return url;
    }
    ...
    public List<DocObject> docContent(DocObject doc, boolean isTitle, String tag) throws NetworkException, ContentException {
        ...
        if (isTitle && "a=a".equals(tag) && ret != null && ret.size() == 0) throw new ContentException("不要再往下了，没有了"); else return ret;
    }
    ...
    static String docContentUrl(DocObject doc, boolean isTitle, DocObject gdoc, String tag) {
        ...
        if (tag != null) url += "&" + tag; else if (!isTitle && doc.isSticky()) url += "&" + HttpConfig.BBS_CON_S_PARAM_NAME;
        return url;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        while ((e = content.indexOf("\n", s)) != -1) {
            ...
            s = e + 1;
            if (c.length() != 0) {
                if (c.startsWith("http://bbs.fudan")) {
                    if (sb.length() != 0) {
