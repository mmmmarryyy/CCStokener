...
public abstract class ModuleLoader {
    ...
    public static ModuleLoader getModuleLoader(URL modURL) throws InvalidFormatException, IOException {
        return getModuleLoader(getType(modURL.toExternalForm()), modURL.toExternalForm(), getData(modURL));
    }

    public static ModuleLoader getModuleLoader(File modFile) throws InvalidFormatException, IOException {
        return getModuleLoader(getType(modFile.getAbsolutePath()), modFile.getAbsolutePath(), getData(modFile));
    }
    ...
    private static int getType(String modName) {
        if (modName.toLowerCase().endsWith(EXT_MOD)) return MOD; else if (modName.toLowerCase().endsWith(EXT_S3M)) return S3M; else if (modName.toLowerCase().endsWith(EXT_XM)) return XM; else if (modName.toLowerCase().endsWith(EXT_STM)) return STM; else if (modName.toLowerCase().endsWith(EXT_IT)) return IT; else if (modName.toLowerCase().endsWith(EXT_ZIP)) {
            return ZIP;
        }
        return UNKNOWN;
    }
    ...
    private static byte[] getData(File file) throws IOException {
        if (!file.exists() || !file.isFile()) throw new IOException("No such file: " + file.getAbsolutePath());
