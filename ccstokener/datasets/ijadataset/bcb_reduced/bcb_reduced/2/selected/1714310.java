    ...
    private Image read1632Bit(boolean is32) throws IOException, BadElementException {
        ...
        if (isBottomUp) {
            ...
            for (int i = height - 1; i >= 0; --i) {
                l = width * 3 * i;
                for (int j = 0; j < width; j++) {
                    if (is32) v = (int) readDWord(inputStream); else v = readWord(inputStream);
            ...
            for (int y = 0; y < height && ptr < values.length; ) {
                ...
                if (count != 0) {
                    ...
                    switch(count) {
                        ...
                        default:
                            if (is8) {
                                for (int i = count; i != 0; --i) val[q++] = (byte) (values[ptr++] & 0xff);
                            } else {
                                int bt = 0;
                                for (int i = 0; i < count; ++i) {
                                    if ((i & 1) == 0) bt = values[ptr++] & 0xff;
                                    val[q++] = (byte) ((i & 1) == 1 ? (bt & 0x0f) : ((bt >>> 4) & 0x0f));
                                }
                            }
                            x += count;
