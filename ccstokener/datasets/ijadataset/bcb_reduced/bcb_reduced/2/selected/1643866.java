...
public class Common implements MicroEmulator, CommonInterface {
    ...
    public void initParams(List params, DeviceEntry defaultDevice, Class defaultDeviceClass) {
        ...
        boolean useConfigFolderForDefaultDevice = true;
        try {
            while (argsIterator.hasNext()) {
                ...
                if ((arg.equals("--help")) || (arg.equals("-help"))) {
                    ...
                    argsIterator.remove();
                } else if (arg.equals("--dontuseconfigfolderfordefaultdevice")) {
                    useConfigFolderForDefaultDevice = false;
        ...
        try {
            if (deviceClass == null) {
                String deviceFileName = defaultDevice.getFileName();
                if (deviceFileName != null) {
                    URL[] urls = new URL[1];
                    if (deviceFileName.startsWith("http") || !useConfigFolderForDefaultDevice) {
                        urls[0] = new URL(deviceFileName);
                    } else {
