...
public class IAXTest extends Applet implements Constants {
    ...
    void showErrorDialog(Frame parent, String message) {
        ...
        Object sel = dlg.showConfirmDialog(topPan, selections, selections[0]);
    }
    ...
    public void setAudioDevices(int inpos, int outpos) {
        ...
        output = out.getDevID();
        trace("inpos:" + inpos + " " + "input:" + input + " " + "outpos:" + outpos + " " + "output:" + output);
    ...
    public void showSettingsDialog(Frame parent) {
        ...
        if (getInputSelection() > -1) inputChooser.select(getInputSelection());
        if (getOutputSelection() > -1) outputChooser.select(getOutputSelection());
    ...
    void showLicenseDialog(Object parent) {
        ...
        Object[] selections = { "Ok" };
        OptionDialog dlg = new OptionDialog(wnd, title);
        Object sel = dlg.showConfirmDialog(lbl, selections, selections[0]);
    }
    ...
    public String getStringParameter(String name, String def) {
        String val;
        if (isApplet) val = getParameter(name); else val = System.getProperty(name);
        if (val != null) {
            return val;
        } else {
            return def;
    ...
    protected void initUI(Container cnt) {
        ...
        textField.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                trace("Pressed: " + code + " " + e.getKeyChar() + " " + e.getKeyText(code));
            }
            ...
            public void keyTyped(KeyEvent e) {
                int code = e.getKeyCode();
                trace("Typed: " + code + " " + e.getKeyChar() + " " + e.getKeyText(code));
            }
        });
    ...
    class Listener extends IAXAdapter {
        ...
        public int textReceived(TextEvent e) {
            ...
            trace("text: " + msg);
            if (msg.startsWith("Unknown event:")) return 1;
        ...
        public int call(CallStateEvent e) {
            ...
            trace("call:" + id + ", " + local + ", " + localName + ", " + remote + ", " + remoteName + " +" + added + " -" + removed + " =" + state);
        ...
        public int netStatsReceived(NetStatsEvent e) {
            trace("netStatsReceived: " + e.getCall() + " " + e.getRtt() + " " + e.getLocal() + " " + e.getRemote());
            return 1;
        }

        public int received(UrlEvent event) {
            trace("received url: " + event.getCall() + " " + event.getType() + " " + event.getUrl());
            URL url = event.getUrl();
            if (isApplet) {
                Object[] selections = { "Ok", "Cancel" };
                OptionDialog dlg = new OptionDialog(wnd, "URL Received");
                Object sel = dlg.showConfirmDialog("Open: " + url + " ?", selections, selections[0]);
                if (sel == "Ok") {
                    trace("Opening: " + url);
