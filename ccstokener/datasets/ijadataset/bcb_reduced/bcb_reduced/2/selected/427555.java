...
public class STFFile extends SimpleParallelSpaceModel {
    ...
    protected static final Logger logger = Logger.getLogger(SimpleParallelSpaceModel.class.getName());
            ...
            for (i = 0; i < tempNumDimensions; i++) {
                ...
                if (!isStringLabel[i]) {
                    try {
                        ...
                        logger.severe("Invalid Number Format: " + nfe.getMessage() + " -> dicarding & setting 0.0f");
                        curVal[j++] = 0.0f;
                    }
                } else {
                    if (label == null) {
