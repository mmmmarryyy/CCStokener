...
public class SRGSGrammar extends Grammar {
    ...
    public SRGSGrammar(boolean showGrammar, boolean optimizeGrammar, boolean addSilenceWords, boolean addFillerWords, Dictionary dictionary) {
    ...
    private GrammarGraph parseRule(RuleComponent rule) throws GrammarException {
        ...
        if (rule instanceof RuleAlternatives) {
            ...
            throw new IllegalArgumentException("Unsupported Rule type: RuleParse: " + rule.toString());
        } else {
    ...
    private GrammarGraph parseRuleReference(RuleReference initialRule) throws GrammarException {
        ...
        GrammarGraph result = (GrammarGraph) ruleStack.contains(initialRule.getRuleName());
        if (result != null) {
            return result;
        } else {
        ...
        if (ruleName == null) {
            throw new GrammarException("Can't resolve " + initialRule + " g " + initialRule.getGrammarReference());
        }
        Rule rule;
        if (ruleName.getGrammarReference() == null) rule = ruleGrammar.getRule(ruleName.getRuleName()); else rule = getExternalGrammar(ruleName.getGrammarReference(), ruleName.getRuleName());
        if (rule == null) {
            throw new GrammarException("Can't resolve grammar reference: " + ruleName.getGrammarReference() + " with grammar name: " + ruleName.getRuleName());
        }
        if (rule == null) {
            throw new GrammarException("Can't resolve rule: " + ruleName.getRuleName());
        }
