...
public class ApplyXSLT extends HttpServlet {
   ...
   */
    public void init(ServletConfig config) throws ServletException {
        ...
        } else CURRENTDIR = System.getProperty("user.dir") + FS;
   ...
   */
    public void process(TransformerFactory tFactory, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SAXException {
        ...
        try {
            if ((xmlSource = getDocument(request, listener)) == null) throw new ApplyXSLTException("getDocument() returned null", new NullPointerException(), response.SC_NOT_FOUND);
        ...
        if (xmlSource != null) {
            try {
                if ((xslSource = getStylesheet(tFactory, request, xmlSource, listener)) == null) {
                    throw new ApplyXSLTException("getStylesheet() returned null", new NullPointerException(), response.SC_NOT_FOUND);
                }
            ...
            if ((xmlSource != null) && (xslSource != null)) {
                try {
                    ...
                    ApplyXSLTException axe = new ApplyXSLTException("Exception occurred during ctor/Transformation:" + EOL + listener.getMessage() + EOL + saxExc.getMessage(), saxExc, response.SC_INTERNAL_SERVER_ERROR);
                    if (debug) writeLog(axe);
                    displayException(response, axe, debug);
                }
            }
            if (debug) {
                ...
                writeLog("  No Conflict Warnings = " + ourDefaultParameters.isNoCW(request) + "  Transformation time: " + time + " ms", response.SC_OK);
            }
        }
    }
    ...
    protected StreamSource getDocument(HttpServletRequest request, ApplyXSLTListener listener) throws ApplyXSLTException {
        try {
            ...
            if ((xmlURL = request.getPathInfo()) != null) {
                ...
                return new StreamSource(new URL("http", ((DefaultApplyXSLTProperties) ourDefaultParameters).getLocalHost(), request.getServerPort(), xmlURL.replace('\\', '/')).openStream());
            }
   ...
   */
    protected StreamSource getStylesheet(TransformerFactory tFactory, HttpServletRequest request, StreamSource xmlSource, ApplyXSLTListener listener) throws ApplyXSLTException {
        try {
            ...
            if (xslURL != null) listener.out.println("Parsing XSL Stylesheet Document from request parameter: " + xslURL); else {
                if (xmlSource != null) {
                ...
                if (xslURL != null) listener.out.println("Parsing XSL Stylesheet Document from XML Document tag: " + xslURL); else if ((xslURL = ourDefaultParameters.getXSLurl(null)) != null) listener.out.println("Parsing XSL Stylesheet Document from configuration: " + xslURL);
            }
   ...
   */
    public String getContentType(Templates templates) {
        ...
        if (media != null) {
            if (encoding != null) return media + "; charset=" + encoding;
            return media;
        } else {
            String method = oprops.getProperty(OutputKeys.METHOD);
            if (method.equals("html")) return "text/html"; else if (method.equals("text")) return "text/plain"; else return "text/xml";
        }
    }
    ...
    public void setStylesheetParams(Transformer transformer, HttpServletRequest request) {
        ...
        while (paramNames.hasMoreElements()) {
            ...
            try {
                ...
                if (paramVals != null) transformer.setParameter(paramName, new XString(paramVals[0]));
            } catch (Exception e) {
            }
        }
        try {
   ...
   */
    protected void writeLog(String msg, int statusCode, Throwable t) {
        if (t == null) writeLog(msg, statusCode); else {
   ...
   */
    protected void displayException(HttpServletResponse response, ApplyXSLTException xse, boolean debug) {
        ...
        if (mesg == null) mesg = ""; else mesg = "<B>" + mesg + "</B>";
        StringTokenizer tokens = new StringTokenizer(mesg, EOL);
        StringBuffer strBuf = new StringBuffer();
        while (tokens.hasMoreTokens()) strBuf.append(tokens.nextToken() + EOL + "<BR>");
                ...
                if (Node.PROCESSING_INSTRUCTION_NODE == child.getNodeType()) {
                    ...
                    if (pi.getNodeName().equals("xml-stylesheet")) {
                        ...
                        if ("text/xsl".equals(pia.getAttribute("type"))) {
                            ...
                            if ((attribute != null) && (attribute.indexOf(attributeValue) > -1)) return tempURL;
   ...
   */
    String getValue(String s) {
        ...
        for (i = 0; i < j; i++) {
            ...
            if (s.indexOf(temp[0]) > -1) return temp[1];
        }
        return "unknown";
    }
}
