...
public class Configuration {
    ...
    private static final String SOLIDBASE_PROPERTIES = "solidbase.properties";

    private static final String SOLIDBASE_DEFAULT_PROPERTIES = "solidbase-default.properties";

    private static final Pattern propertyPattern = Pattern.compile("^connection\\.([^\\s\\.]+)\\.(driver|url|username|password)$");
    ...
    public String getFirstError() {
        ...
        if (database != null) {
            if (StringUtils.isBlank(database.driver)) return "Missing 'connection.driver' or -driver";
            if (StringUtils.isBlank(database.url)) return "Missing 'connection.url' or -url";
            if (StringUtils.isBlank(database.userName)) return "Missing 'connection.username' or -username";
            if (StringUtils.isBlank(this.upgradeFile) && StringUtils.isBlank(this.sqlFile)) return "Missing 'upgrade.file', -upgradefile, 'sql.file' or -sqlfile";
            if (!StringUtils.isBlank(this.sqlFile) && !StringUtils.isBlank(this.target)) return "'upgrade.target', -target is not allowed in combination with 'sql.file', -sqlfile";
            for (Database secondary : this.secondaryDatabases.values()) {
                if (StringUtils.isWhitespace(secondary.driver)) return "Missing 'connection." + secondary.name + ".driver'";
                if (StringUtils.isWhitespace(secondary.url)) return "Missing 'connection." + secondary.name + ".url'";
                if (StringUtils.isBlank(secondary.userName)) return "Missing 'connection." + secondary.name + ".username'";
            }
        } else {
            if (!StringUtils.isBlank(this.sqlFile)) return "'sql.file' property or -sqlfile commandline option specified but no database configured";
            if (!StringUtils.isBlank(this.upgradeFile)) return "'upgrade.file' property or -upgradefile commandline option specified but no database configured";
            if (!StringUtils.isBlank(this.target)) return "'upgrade.target' property or -target commandline option specified but no database configured";
        }
        return null;
    }
