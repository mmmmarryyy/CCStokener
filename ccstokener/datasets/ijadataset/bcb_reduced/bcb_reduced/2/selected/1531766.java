...
public final class Service {
    ...
    static {
        try {
            ...
            if (configFile != null) loadProperties();
            initializeProperties();
        } catch (IOException e) {
            logger.log(Level.SEVERE, Localizer.getLocalizedMessage(Service.class, "initProperties.error"), e);
        }
    }
	 ...
	 */
    public static Device[] listDevicesByType(DeviceType type) throws FTD2xxException {
        List<Device> devices = new ArrayList<Device>();
        for (Device d : listDevices()) {
            if (d.getType() == type) devices.add(d);
        }
	 ...
	 */
    public static Device[] listDevicesByDescription(String description) throws FTD2xxException {
        List<Device> devices = new ArrayList<Device>();
        for (Device d : listDevices()) {
            try {
                DeviceDescriptor desc = d.getDeviceDescriptor();
                if (desc.getProductDescription().equals(description)) devices.add(d);
            } catch (FTD2xxException e) {
                logger.log(Level.SEVERE, "Failed to access device descriptor.", e);
            }
        }
        return devices.toArray(new Device[devices.size()]);
    }
	 ...
	 *             if the devices can not be listed.
	 */
    public static Device[] listDevicesBySerialNumber(String serialNumber) throws FTD2xxException {
        List<Device> devices = new ArrayList<Device>();
        for (Device d : listDevices()) {
	 ...
	 *             started).
	 */
    private static native void startInputTask(long handle) throws FTD2xxException;
	 ...
	 *             stopped).
	 */
    private static native void stopInputTask(long handle) throws FTD2xxException;
