...
public class AddFriendWizard extends JWizard {
    ...
    public void XUILayoutComplete(final UISubsystem ui, XUIDialog outerDialog) {
        ...
        if (new ForwardInvitationNodesList.ForwardInvitationListModel(ui).getSize() == 0 || ui.getCore().getFriendManager().friends().size() == 0) innerXUI.getComponent("radio1_3").setEnabled(false);
    ...
    public static AddFriendWizard open(UISubsystem ui, int startAtStep) throws Exception {
        ...
        if (startAtStep == STEP_FORWARD_INVITATIONS) wizard.goToForwardInvitations(); else if (startAtStep == STEP_ATTEMPT_CONNECT) wizard.goToAttemptConnect(); else if (startAtStep == STEP_PORT_OPEN_TEST) wizard.goToPortTest(); else if (startAtStep != STEP_INTRO) throw new Exception("No support for starting at step " + startAtStep);
        return wizard;
    }
