...
public class WinRegistryWrapper {
     ...
     */
    private static native int RegSetValueEx(int hKey, byte[] valueName, byte[] value);
     ...
     */
    private static native byte[] RegEnumKeyEx(int hKey, int subKeyIndex, int maxKeyLength);

    /**
     * Java wrapper for Windows registry API RegEnumValue()
     */
    private static native byte[] RegEnumValue(int hKey, int valueIndex, int maxValueNameLength);
    ...
    public static int WinRegDeleteKey(int hKey, String subKey) {
        ...
        if (result == ERROR_SUCCESS) {
            ...
            if (res == ERROR_ITEM_NOTEXIST) {
                ...
                String subSubKey;
                String[] subSubKeys = WinRegGetSubKeys(hKey, subKey, MAX_KEY_LENGTH);
    ...
    public static String WinRegQueryValueEx(int hKey, String subKey, String valueName) {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            valueBytes = RegQueryValueEx(openResult[OPENED_KEY_HANDLE], lpValueName);
            RegCloseKey(openResult[OPENED_KEY_HANDLE]);
            if (valueBytes != null) {
    ...
    public static int WinRegSetValueEx(int hKey, String subKey, String valueName, String value) {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            int setResult = RegSetValueEx(openResult[OPENED_KEY_HANDLE], lpValueName, lpValue);
            RegCloseKey(openResult[OPENED_KEY_HANDLE]);
            return setResult;
        }
    }
    ...
    public static int WinRegDeleteValue(int hKey, String subKey, String valueName) {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            byte[] lpValueName = stringToByteArray(valueName);
            int deleteResult = RegDeleteValue(openResult[OPENED_KEY_HANDLE], lpValueName);
            RegCloseKey(openResult[OPENED_KEY_HANDLE]);
            return deleteResult;
        }
    }
    ...
    public static String WinRegEnumKeyEx(int hKey, String subKey, int subKeyIndex, int maxKeyLength) {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            byte[] keyBytes = RegEnumKeyEx(openResult[OPENED_KEY_HANDLE], subKeyIndex, maxKeyLength);
            RegCloseKey(openResult[OPENED_KEY_HANDLE]);
            if (keyBytes != null) {
    ...
    public static String WinRegEnumValue(int hKey, String subKey, int valueIndex, int maxValueNameLength) {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            byte[] valueBytes = RegEnumValue(openResult[OPENED_KEY_HANDLE], valueIndex, maxValueNameLength);
            RegCloseKey(openResult[OPENED_KEY_HANDLE]);
            if (valueBytes != null) {
                return byteArrayToString(valueBytes);
            } else {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            if (subKeysNum == 0) {
                ...
                for (int subKeyIndex = 0; subKeyIndex < subKeysNum; subKeyIndex++) {
                    keyBytes = RegEnumKeyEx(openResult[OPENED_KEY_HANDLE], subKeyIndex, maxKeyLength);
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            if (valuesNum == 0) {
                ...
                for (int valueIndex = 0; valueIndex < valuesNum; valueIndex++) {
                    valueBytes = RegEnumValue(openResult[OPENED_KEY_HANDLE], valueIndex, maxValueLength);
    ...
    public static int WinRegValueExist(int hKey, String subKey, String valueName) {
        ...
        if (openResult[ERROR_CODE] != ERROR_SUCCESS) {
            ...
            byte[] lpValueName = stringToByteArray(valueName);
            byte[] valueBytes = RegQueryValueEx(openResult[OPENED_KEY_HANDLE], lpValueName);
            RegCloseKey(openResult[OPENED_KEY_HANDLE]);
            if (valueBytes == null) {
                return ERROR_ITEM_NOTEXIST;
            } else {
