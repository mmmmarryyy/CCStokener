...
/****************************************************
 * PokerApp is the application that will start the a single human player client.  It is
 * used to conenct to a server and play a game of poker.  
 *
 * @author Dan Puperi
 * @version 1.00
 *
 ...
 **/
    ...
    /**
 * If StartPoker was used to start this, the instance of the StartPoker class
 **/
    public StartPoker startUpApp;

    /**
 * If PokerApplet was used to start this, the instance of the PokerApplet class
 **/
 ...
 **/
    ...
    private PokerFrame mainWindow;

    private PokerApp theApp;
    ...
    private ArrayList cardBackLabels;

    private ArrayList cardBackFiles;

    private ArrayList tableImgLabels;

    private ArrayList tableImgFiles;

    private int numGamesPlayed;

    private javax.swing.Timer timer;

    private String appletServer;

    private String appletPort;

    /***************************
 * The PokerApp constructor if this class is created by the StartPoker class
 *
 * @param a The StartPoker instance to which this PokerApp belongs
 * @param fname The input file from which inputs and settings are read
 *
 **/
    ...
    /***************************
 * The PokerApp constructor if this class is created by the PokerApplet class
 *
 * @param a The PokerApplet instance to which this PokerApp belongs
 * @param fname The input file from which inputs and settings are read
 *
 **/
    ...
    /***********************
 * The init() function initializes all of the variables and set up the application.
 * It is run immediately after the creation of the class instance.
 **/
    public void init() {
 ...
 *
 * @param log_writer This function is passed the log_writer so the results can be written to the log file.
 *
 **/
 ...
 *
 * @param name The players name who is trying to connect
 * @param host The string representation of the host - either and IP address or acceptable hostname
 * @param p The port number on which to connect
 *
 **/
    public void connectSocket(String name, String host, String p) {
        log("PokerApp.connectSocket( " + name + ", " + host + ", " + p + " )", 3);
        hostName = host;
        try {
            ...
            if (clientSocket == null) {
                log("ERROR : unable to connect to the socket - " + hostName + " port = " + port);
                mainWindow.sendMessage("Error - unable to connect to the socket");
            } else {
                mainWindow.sendMessage("Connection to " + hostName + " successful.  Choose a seat to join the game.");
 ...
 * connectSocket() is used to connect to a server
 *
 * @param redraw Whether or not the client window should be redrawn
 *
 **/
    public void disconnectSocket(boolean redraw) {
        ...
        try {
            if (clientSocket != null) {
                ...
                client.stopRunning();
                clientSocket.close();
                if (redraw && (mainWindow != null)) mainWindow.sendMessage("Disconnected from " + hostName);
            }
            clientSocket = null;
            if (redraw) {
                resetTable();
                ...
                inGame = false;
                structuredRaiseEnable = false;
                if (mainWindow != null) mainWindow.menuRedisplay();
                if (mainWindow != null) mainWindow.disableButtons();
                gameLabels = new ArrayList();
                gameClasses = new ArrayList();
                ruleNames = new ArrayList();
                ruleValues = new ArrayList();
                playerList = new ArrayList();
                if (thisPlayer != null) thisPlayer.seat = -9;
            }
        } catch (Exception e) {
 ...
 **/
    ...
    /***********************
 * The resetTable() function is normally called in between games to clean up the table.
 **/
 ...
 *
 * @param name The name of the player who folder
 *
 **/
 ...
 * 
 * return The server string of the applet (if this is being run as an applet)
 **/
 ...
 * 
 * return The port that this applet is connected to (if this is being run as an applet)
 **/
 ...
 * 
 * @param title The title of the error box window
 * @param text The text of the error message
 * @param exit Whether or not this error will result in the application exiting after error box is OK'ed
 *
 **/
    ...
    /***********************
 * PokerErrorBox class defines the error box that will be displayed by the displayError() function.
 **/
    ...
    /***********************  
 * displaySidePotMessages() is used to display the results of splitting up the side pots.
 **/
    public void displaySidePotMessages() {
        if (messageWindow == null) {
            SidePotMessageBox sidePotWindow = new SidePotMessageBox(sidePotMessages);
            sidePotWindow.setBounds(60, 200, 400, 75 + 20 * sidePotMessages.size());
            sidePotWindow.setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);
            sidePotWindow.setVisible(true);
        }
    }

    /***********************
 * SidePotMessageBox class defines the message box that can be displayed to announce side pot winners.
 **/
