...
public class PngImage {
                ...
                switch(colorType) {
                    case 0:
                        if (len >= 2) {
                            len -= 2;
                            int gray = getWord(is);
                            if (bitDepth == 16) transRedGray = gray; else additional.put(PdfName.MASK, new PdfLiteral("[" + gray + " " + gray + "]"));
                        }
                        break;
                    case 2:
                        if (len >= 6) {
                            ...
                            if (bitDepth == 16) {
                                ...
                                transGreen = green;
                                transBlue = blue;
                            } else additional.put(PdfName.MASK, new PdfLiteral("[" + red + " " + red + " " + green + " " + green + " " + blue + " " + blue + "]"));
                        }
                        break;
                    case 3:
                        if (len > 0) {
    ...
    PdfObject getColorspace() {
        if (icc_profile != null) {
            if ((colorType & 2) == 0) return PdfName.DEVICEGRAY; else return PdfName.DEVICERGB;
        }
        if (gamma == 1f && !hasCHRM) {
            if ((colorType & 2) == 0) return PdfName.DEVICEGRAY; else return PdfName.DEVICERGB;
        } else {
    ...
    Image getImage() throws IOException {
        ...
        try {
            ...
            if (bpc == 16) bpc = 8;
            Image img;
            if (image != null) img = Image.getInstance(width, height, components, bpc, image); else {
            ...
            if (additional.get(PdfName.COLORSPACE) == null) additional.put(PdfName.COLORSPACE, getColorspace());
    ...
    void decodeIdat() {
        ...
        if (palShades) smask = new byte[width * height]; else if (genBWMask) smask = new byte[(width + 7) / 8 * height];
    ...
    void processPixels(byte curr[], int xOffset, int step, int y, int width) {
        ...
        if (palShades) {
            if ((colorType & 4) != 0) {
                if (bitDepth == 16) {
                    for (int k = 0; k < width; ++k) out[k * inputBands + sizes] >>>= 8;
                }
                ...
                for (srcX = 0; srcX < width; srcX++) {
                    int idx = out[srcX];
                    int r = 255;
                    if (idx < trans.length) v[0] = trans[idx];
                ...
                case 3:
                    {
                        ...
                        for (srcX = 0; srcX < width; srcX++) {
                            int idx = out[srcX];
                            int r = 0;
                            if (idx < trans.length) v[0] = (trans[idx] == 0 ? 1 : 0);
            ...
            case 8:
                {
                    ...
                    for (int k = 0; k < out.length; ++k) out[k] = curr[k] & 0xff;
                    return out;
                }
            case 16:
                {
                    int out[] = new int[curr.length / 2];
                    for (int k = 0; k < out.length; ++k) out[k] = ((curr[k * 2] & 0xff) << 8) + (curr[k * 2 + 1] & 0xff);
                    return out;
                }
            default:
                {
