...
public abstract class SecureRenderer {
    ...
    private static final Map<CodeSource, Map<ClassLoader, SoftReference<SecureRenderer>>> renderers = new WeakHashMap<CodeSource, Map<ClassLoader, SoftReference<SecureRenderer>>>();

    public abstract void render(Environment e, TemplateElement t) throws TemplateException, IOException;
     ...
     * element throws the exception
     */
    static void renderSecurely(final CodeSource codeSource, Environment env, TemplateElement t) throws TemplateException, IOException {
        ...
        synchronized (classLoaderMap) {
            ...
            if (renderer == null) {
                try {
                    renderer = AccessController.doPrivileged(new PrivilegedExceptionAction<SecureRenderer>() {
                        ...
                        public SecureRenderer run() throws Exception {
                            ...
                            Class c = secCl.defineAndLinkClass(SecureRenderer.class.getName() + "Impl", secureInvokerImplBytecode, codeSource);
                            return (SecureRenderer) c.newInstance();
                        }
                    });
                    classLoaderMap.put(classLoader, new SoftReference<SecureRenderer>(renderer));
