...
public class AWSAuthConnection {
    ...
    public HttpURLConnection getOutputStream(String bucket, String key, Map headers) throws IOException {
        HttpURLConnection request = makeRequest("PUT", bucket + "/" + Utils.urlencode(key), headers, null);
        request.setDoOutput(true);
        return request;
    }
     ...
     * @param key     The name of the key to use.
     * @param headers A Map of String to List of Strings representing the http
     * @param out     OutputStream will be used for writing the S3 object
     *                headers to pass (can be null).
     */
    public GetResponse get(String bucket, String key, Map headers, OutputStream out) throws MalformedURLException, IOException {
        return new GetResponse(makeRequest("GET", bucket + "/" + Utils.urlencode(key), headers), out);
    }
     ...
     *                headers to pass (can be null).
     */
    public GetResponse getStream(String bucket, String key, Map headers) throws MalformedURLException, IOException {
        return new GetResponse(makeRequest("GET", bucket + "/" + Utils.urlencode(key), headers), false);
    }
     ...
     *                headers to pass (can be null).
     */
    public Response head(String bucket, String key, Map headers) throws IOException {
        return new Response(makeRequest("HEAD", bucket + "/" + Utils.urlencode(key), headers));
    }

    public Response head(String bucket, Map headers) throws IOException {
        return new Response(makeRequest("HEAD", bucket, headers));
    }
    ...
    public GetResponse getACL(String bucket, String key, Map headers) throws MalformedURLException, IOException {
        if (key == null) key = "";
        return new GetResponse(makeRequest("GET", bucket + "/" + Utils.urlencode(key) + "?acl", headers));
    }
    ...
    public Response putACL(String bucket, String key, String aclXMLDoc, Map headers) throws MalformedURLException, IOException {
        ...
        HttpURLConnection request = makeRequest("PUT", bucket + "/" + Utils.urlencode(key) + "?acl", headers, object);
        request.setDoOutput(true);
     ...
     *                headers to pass (can be null).
     */
    public ListAllMyBucketsResponse listAllMyBuckets(Map headers, String path) throws MalformedURLException, IOException {
        return new ListAllMyBucketsResponse(makeRequest("GET", path, headers));
    }
     ...
     * @param prefix All returned keys will start with this string (can be null).
     */
    public ListAllMyBucketsResponse listAllMyBuckets(String prefix) throws MalformedURLException, IOException {
        String path = Utils.pathForListOptions("", prefix, null, null, null);
        return listAllMyBuckets(null, path);
    }

    /**
     * Make a new HttpURLConnection without passing an S3Object parameter.
     */
    private HttpURLConnection makeRequest(String method, String resource, Map headers) throws IOException {
