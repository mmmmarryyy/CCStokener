 ...
 *
 * @version $Id: SectorTextReader.java 8 2009-06-13 18:01:14Z tjoneslow $
 */
public abstract class SectorTextReader implements AstrogationFileReader {
     ...
     * can not be found.
     */
    public void read() throws FileNotFoundException, IOException, SECFileStateMachineException, JiBXException {
    ...
    protected void readDataRegex(Reader r) throws IOException, SECFileStateMachineException {
        ...
        UWP uwp;
        if (r instanceof BufferedReader) read = (BufferedReader) r; else read = new BufferedReader(r);
        line = read.readLine();
        while (line != null) {
            if (line.length() < 64) {
                line = read.readLine();
            ...
            if (m.group(5).trim().length() > 2) thisSystem.setTradeCodes(m.group(5).trim()); else {
        ...
        while (parser.ttype != StreamTokenizer.TT_EOF) {
            ...
            switch(state) {
                ...
                case NAME_NUMBER:
                    ...
                    if (thisSystem.getName().length() > 40) throw new SECFileStateMachineException("System name: " + thisSystem.getName() + " is too long at line: " + parser.lineno());
                    break;
                case HEXID:
                ...
                case REMARK:
                    if (parser.ttype == StreamTokenizer.TT_NUMBER) {
                        ...
                        if (thisSystem.getTradeCodes() == null) {
                            thisSystem.setTradeCodes(parser.sval);
                        } else {
                            thisSystem.setTradeCodes(thisSystem.getTradeCodes() + " " + parser.sval);
                        }
                    }
                    break;
                case ZONE:
    ...
    void readPBGCode(StarSystem thisSystem) throws SECFileStateMachineException {
        if (parser.ttype != StreamTokenizer.TT_NUMBER) throw new SECFileStateMachineException("State machine failure: Expecting PBG code but not found at line: " + parser.lineno());
    ...
    void readPolityCode(StarSystem thisSystem) throws SECFileStateMachineException {
        if (parser.ttype != StreamTokenizer.TT_WORD) throw new SECFileStateMachineException("State machine failure: Expecting Allegiance code but not found at line: " + parser.lineno());
