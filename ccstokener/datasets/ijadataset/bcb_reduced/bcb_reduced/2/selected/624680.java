...
public class SeleniumServer {
    ...
    public void stop() {
        ...
        if (numTries > MAX_SHUTDOWN_RETRIES) {
    ...
    protected void runHtmlSuite() {
        ...
        try {
            ...
            result = launcher.runHTMLSuite(getRequiredSystemProperty("htmlSuite.browserString"), startURL, suiteFile, resultFile, configuration.getTimeoutInSeconds(), (!configuration.isSingleWindow()));
    ...
    private void logStartupInfo() throws IOException {
        ...
        if (null != configuration.getForcedBrowserMode()) {
            LOGGER.info("\"" + configuration.getForcedBrowserMode() + "\" will be used as the browser " + "mode for all sessions, no matter what is passed to getNewBrowserSession.");
        }
    }
