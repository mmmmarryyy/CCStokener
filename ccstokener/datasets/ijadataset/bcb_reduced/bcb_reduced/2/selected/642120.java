...
public class UpdateManagerDefault implements UpdateManager {
    ...
    private String server = ConfigurationMgr.getString(UpdateComponent.NAME, UpdateConfigurationConstants.SERVER);

    private int port = ConfigurationMgr.getInt(UpdateComponent.NAME, UpdateConfigurationConstants.PORT);

    private String updatePath = ConfigurationMgr.getString(UpdateComponent.NAME, UpdateConfigurationConstants.SERVER_UPDATE_PATH);

    private String updateDoc = ConfigurationMgr.getString(UpdateComponent.NAME, UpdateConfigurationConstants.UPDATEDOC);

    private File localUpdateRepository = new File(Utils.getSystemTempFolder(), ConfigurationMgr.getString(UpdateComponent.NAME, UpdateConfigurationConstants.LOCAL_UPDATE_FOLDER));
	 ...
	 */
    public synchronized ModuleUpdateDescriptor[] getAvailableUpdates(Module module) throws XMLException, IOException {
        ...
        return (ModuleUpdateDescriptor[]) m.values().toArray(new ModuleUpdateDescriptor[m.size()]);
    }
