...
public class DetermineRouteProvider implements IOperationProvider {
    ...
    /** The Logger. */
    private static Logger LOGGER = Logger.getLogger(DetermineRouteProvider.class.getName());
    ...
    private void parseRequest(OperationContext context, Node ndReq, XPath xpath) throws Exception {
        ...
        if (ndRoutePlan != null) {
            ...
            if (ndWayPointList != null) {
                ...
                if (ndStartPoint != null) {
                    ...
                    Node ndStartPos = (Node) xpath.evaluate("xls:Position/gml:Point/gml:pos", ndStartPoint, XPathConstants.NODE);
                ...
                if (ndViaPoint != null) {
                    ...
                    Point point = new Point();
                    Node ndViaPos = (Node) xpath.evaluate("xls:Position/gml:Point/gml:pos", ndViaPoint, XPathConstants.NODE);
                    if (ndViaPos != null) {
                ...
                if (ndEndPoint != null) {
                    ...
                    Node ndEndPos = (Node) xpath.evaluate("xls:Position/gml:Point/gml:pos", ndEndPoint, XPathConstants.NODE);
                    Point point = new Point();
                    if (ndEndPos != null) {
            ...
            if (ndAvoidList != null) {
                ...
                Node ndStartPos = (Node) xpath.evaluate("xls:Position/gml:Point/gml:pos", ndAvoidList, XPathConstants.NODE);
                Point point = new Point();
                if (ndStartPos != null) {
                    String[] coords = ndStartPos.getTextContent().split(" ");
                    point.setX(coords[0]);
        ...
        if (ndRouteInst != null) {
            ...
            String provideGeometry = (String) xpath.evaluate("@provideGeometry", ndRouteInst, XPathConstants.STRING);
	 ...
	 * @throws Exception
	 */
    private void geocode(OperationContext context, Node ndReq, XPath xpath, Point point) throws Exception {
        ...
        try {
            ArrayList<GeocodedAddress> ga = geocodeProvider.executeRequest(context, reqParams);
    ...
    private DetermineRouteParams parseRouteResponse(String sResponse) throws Throwable {
        DetermineRouteParams routeParams = null;
        try {
            routeParams = new DetermineRouteParams();
            JSONObject jsonResp = new JSONObject(sResponse);
            String xResponse = "<?xml version='1.0'?><response>" + org.json.XML.toString(jsonResp) + "</response>";
            ...
            if (jsonResp != null) {
                ...
                if (routes != null) {
                    ...
                    JSONObject geometry = routesFeatures.getJSONObject(0).getJSONObject("geometry");
                    JSONArray paths = geometry.getJSONArray("paths").getJSONArray(0);
                    for (int i = 0; i < paths.length(); i++) {
                        GmlPos pos = new GmlPos();
                        pos.setPoint(paths.getString(i).replace("[", "").replace("]", "").replace(",", " "));
                    }
                }
                ...
                if (directions != null) {
                    ...
                    rteSummary.setTotalDistance(summary.getString("totalLength"));
                    rteSummary.setTotalDriveTime(summary.getString("totalDriveTime"));
                    String routeName = direction.getString("routeName");
                    String desc = direction.getString("summary");
                    coords = summary.getString("envelope");
                    if (coords != null && !coords.equals("")) rteSummary.setBbox(new BBoxContext(coords));
                    ...
                    if (rteSummary.getBbox() == null) rteSummary.setBbox(new BBoxContext(minx, miny, maxx, maxy));
                    JSONArray directionFeatures = direction.getJSONArray("features");
                    ArrayList<RouteInstruction> routeInstructionList = new ArrayList<RouteInstruction>();
                    for (int j = 0; j < directionFeatures.length(); j++) {
                        JSONObject featuresArray = directionFeatures.getJSONObject(j);
                        JSONObject attributes = featuresArray.getJSONObject("attributes");
                        String compressedGeom = featuresArray.getString("compressedGeometry");
     ...
     * @throws Throwable
     */
    private void makeExportMapRequest(DetermineRouteParams determineRouteParams, String srvCfgMapService) throws Throwable {
        ImageOutput imgOutput = determineRouteParams.getRouteMap().getImageOutput();
        Output output = imgOutput.getOutput();
        determineRouteParams.getRouteMap().getImageOutput().setBboxContext(determineRouteParams.getRouteSummary().getBbox());
        String requestParameters = "";
        requestParameters += "format=" + output.getFormat() + "&";
        requestParameters += "size=" + output.getWidth() + "," + output.getHeight() + "&";
        if (imgOutput.getBboxContext() != null) requestParameters += "bboxSR=4326" + "&"; else requestParameters += "bboxSR=4326&";
        ...
        imgOutput.setUrl(srvCfgMapService + "/export?" + Val.escapeXml(requestParameters));
    }
}
