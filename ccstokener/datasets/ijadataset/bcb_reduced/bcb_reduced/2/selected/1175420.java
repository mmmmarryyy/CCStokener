...
public class AMFConnection {
    ...
    public int getObjectEncoding() {
        if (!objectEncodingSet) return getDefaultObjectEncoding();
    ...
    public void addAmfHeader(String name, boolean mustUnderstand, Object data) {
        if (amfHeaders == null) amfHeaders = new ArrayList<MessageHeader>();
    ...
    public void removeAllAmfHeaders() {
        if (amfHeaders != null) amfHeaders = null;
    }
    ...
    public void addHttpRequestHeader(String name, String value) {
        if (httpRequestHeaders == null) httpRequestHeaders = new HashMap<String, String>();
    ...
    public void removeAllHttpRequestHeaders() {
        if (httpRequestHeaders != null) httpRequestHeaders = null;
    }
    ...
    public Object call(String command, Object... arguments) throws ClientStatusException, ServerStatusException {
        ...
        if (amfHeaders != null) {
            for (MessageHeader header : amfHeaders) requestMessage.addHeader(header);
        }
        ...
        try {
            ...
            if (e instanceof ClientStatusException) throw (ClientStatusException) e;
            if (e instanceof ServerStatusException) throw (ServerStatusException) e;
    ...
    protected void processHttpResponseHeaders() {
        ...
        for (Map.Entry<String, List<String>> element : headers.entrySet()) {
            ...
            for (String headerValue : headerValues) {
                if (SET_COOKIE.equals(headerName) || COOKIE.equals(headerName) || SET_COOKIE2.equals(headerName) || COOKIE2.equals(headerName)) processSetCookieHeader(headerValue);
            }
        }
    }
    ...
    protected void processAmfHeaders(ArrayList<MessageHeader> headers) {
        if (amfHeaderProcessor == null) return;
        for (MessageHeader header : headers) amfHeaderProcessor.processHeader(header);
    }
    ...
    protected void processSetCookieHeader(String headerValue) {
        ...
        if (cookie.indexOf(COOKIE_SEPERATOR) > 0) cookie = headerValue.substring(0, cookie.indexOf(COOKIE_SEPERATOR));
    ...
    protected void setHttpRequestCookieHeader() {
        ...
        for (Map.Entry<String, String> element : cookies.entrySet()) {
            ...
            String value = element.getValue();
            if (cookieHeaderValue == null) cookieHeaderValue = new StringBuffer(name + COOKIE_NAMEVALUE_SEPERATOR + value); else cookieHeaderValue.append(COOKIE_SEPERATOR + " " + name + COOKIE_NAMEVALUE_SEPERATOR + value);
        }
        if (cookieHeaderValue != null) urlConnection.setRequestProperty(COOKIE, cookieHeaderValue.toString());
    }
