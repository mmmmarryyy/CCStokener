...
public class SmoothTilePainter extends SimplePaintVisitor {
    ...
    */
    ...
    public void visitAll(final IDataSet aData, final Graphics2D aGraphics) {
        ...
        if (getNavigatableComponent().getZoom() <= lod1ZoomLevel) {
            if (data instanceof LODDataSet) {
                ...
                LOG.log(Level.WARNING, "SmoothTilePainter was ordered to draw a" + " very low zoom-level of " + getNavigatableComponent().getZoom() + " but the map" + " is no LODDataSet! This may be very,very slow.");
            }
        }
    ...
    protected int getBitmapHeight(final BufferedImage bmp, final int defaultHeight) {
        if (bmp == null) return defaultHeight;
    ...
    protected int getBitmapWidth(final BufferedImage bmp, final int defaultWidth) {
        if (bmp == null) return defaultWidth;
        return bmp.getWidth();
    }
    ...
    protected Tile getTile(final LatLon latLon, final int zoom) throws IOException {
        return getTile(SmoothTilePainter.getXTileNumber(latLon.lat(), latLon.lon(), zoom), SmoothTilePainter.getYTileNumber(latLon.lat(), latLon.lon(), zoom), zoom);
    }
    ...
    protected Tile getTile(final int xTileNr, final int yTileNr, final int zoom) throws IOException {
        try {
            ...
            while (myTileCache.size() <= zoom) {
                myTileCache.add(new HashMap<Integer, HashMap<Integer, Reference<Tile>>>());
            }
            ...
            if (reference != null) retval = reference.get();
            if (retval == null) {
                if (zoom >= 0 && xTileNr >= 0 && yTileNr >= 0 && xTileNr < (1 << zoom) && yTileNr < (1 << zoom)) {
                    retval = new Tile(xTileNr, yTileNr, zoom, true);
                    yMap.put(yTileNr, new java.lang.ref.SoftReference<Tile>(retval));
                    if (myTileCacheLRU.size() >= TILECACHELRUSIZE) myTileCacheLRU.remove(myTileCacheLRU.keySet().iterator().next());
            ...
            LOG.log(Level.SEVERE, "[RuntimeException] Problem in getTile()", e);
            return null;
        }
    }
    ...
    private void repaintTile(final Tile t) {
        ...
        if (nc2 instanceof java.awt.Component) ((java.awt.Component) nc2).repaint();
    }
     ...
     * gets tight.
     */
    private ArrayList<HashMap<Integer, HashMap<Integer, Reference<Tile>>>> myTileCache = new ArrayList<HashMap<Integer, HashMap<Integer, Reference<Tile>>>>(MAXZOOM);
    ...
    public class DownloadTileTask implements Callable<BufferedImage> {
        ...
        public DownloadTileTask(final URL downloadUrl) {
            if (downloadUrl == null) throw new IllegalArgumentException("null url given");
    ...
    public static int getYTileNumber(final double lat, final double lon, final int zoom) {
        int zoomFactor = (1 << zoom);
        int ytile = (int) Math.floor((1 - Math.log(Math.tan(lat * Math.PI / HALFCIRCLE) + 1 / Math.cos(lat * Math.PI / HALFCIRCLE)) / Math.PI) / 2 * zoomFactor);
        return ytile;
    }
    ...
    protected class Tile {
        ...
        private final class AsyncTimeLoader implements Runnable {
            ...
            private final class BufferdImageNavComponent implements INavigatableComponent {
                ...
                public BufferdImageNavComponent() {
                    ...
                    center = new EastNorth(min.east() + (max.east() - min.east()) / 2, min.north() + (max.north() - min.north()) / 2);
                ...
                public EastNorth getEastNorth(final int x, final int y) {
                    return new EastNorth(center.east() + (x - getWidth() / 2.0) * scaleX, center.north() - (y - getHeight() / 2.0) * scaleY);
                }
                ...
                public LatLon getLatLon(final int x, final int y) {
                    EastNorth eastNorth = new EastNorth(center.east() + (x - getWidth() / 2.0) * scaleX, center.north() - (y - getHeight() / 2.0) * scaleY);
             ...
             * exception.
             */
            private AsyncTimeLoader(final int aTile, final int aZoom, final int aTile2, final boolean allowIllegalCoords) {
                myTile = aTile;
                myZoom = aZoom;
                myTile2 = aTile2;
                if (!allowIllegalCoords) {
                    if (myZoom < 1 || myTile2 < 0 || myTile < 0 || myTile2 >= 1 << myZoom || myTile >= 1 << myZoom) throw new IllegalArgumentException("Tile out of range! " + "zoom=" + myZoom + " x=" + myTile + " y=" + myTile2);
                }
            }
        ...
        public double getWidth() {
            if (projected == null) projected = projectTileNumber(getXtile(), getYtile(), getZoomLevel());
        ...
        public Bounds getBounds() {
            if (projected == null) projected = projectTileNumber(getXtile(), getYtile(), getZoomLevel());
            return new Bounds(projected[PROJECTED_SOUTH], projected[PROJECTED_WEST], projected[PROJECTED_NORTH], projected[PROJECTED_EAST]);
        }
        ...
        private String getTileDirName() {
            return Settings.getInstance().get("renderedTileCache.dir", Settings.getInstance().get("map.dir", System.getProperty("user.home") + File.separator + ".openstreetmap") + File.separator + "tiles" + File.separator);
        }
         ...
         * @throws IOException if javax.imageio has a problem
         */
        public Tile(final double lat, final double lon, final int zoom, final boolean allowIllegalCoords) throws IOException {
            this(getXTileNumber(lat, lon, zoom), getYTileNumber(lat, lon, zoom), zoom, allowIllegalCoords);
        }
         ...
         * @throws IOException if javax.imageio has a problem
         */
        public Tile(final int xTile, final int yTile, final int zoom, final boolean allowIllegalCoords) throws IOException {
            SmoothTilePainter.this.myExecutorService.execute(new AsyncTimeLoader(yTile, zoom, xTile, allowIllegalCoords));
            ...
            LOG.log(Level.FINE, "new Tile(xtile=" + myXTileNumber + ", ytile=" + myYTileNumber + " zoom=" + zoom + ") => x=" + myXTileNumber + " y=" + myYTileNumber + " projected=" + Arrays.toString(projected));
        }
        ...
        protected double[] projectTileNumber(final int x, final int y, final int zoom) {
            ...
            return (new double[] { lat2, long1, lat1, long1 + unit });
        }
