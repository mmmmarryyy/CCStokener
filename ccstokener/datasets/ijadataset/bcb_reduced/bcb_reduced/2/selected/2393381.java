...
public final class PngImage implements ImageProducer {
    ...
    private synchronized void produceHelper(ImageConsumer[] ics) {
        try {
            ...
            if (data.pixels == null) {
                ...
                setHints(ics);
                for (int i = 0; i < ics.length; i++) {
                    ics[i].setPixels(0, 0, data.header.width, data.header.height, data.header.model, data.pixels, 0, data.header.width);
    ...
    private void setHints(ImageConsumer[] ics) {
        for (int i = 0; i < ics.length; i++) {
            if (progressive && data.pixels == null && (data.header.interlace != INTERLACE_TYPE_NONE)) {
                ...
                ics[i].setHints(ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.SINGLEPASS | ImageConsumer.SINGLEFRAME | ImageConsumer.COMPLETESCANLINES);
            }
        }
    }

    private void produceNonInterlaced(ImageConsumer[] ics, PixelInputStream pis) throws IOException {
    ...
    private void produceInterlaced(ImageConsumer[] ics, PixelInputStream pis) throws IOException {
        int w = data.header.width, h = data.header.height;
        data.pixels = new int[w * h];
        int[] rowbuf = new int[w + 8];
