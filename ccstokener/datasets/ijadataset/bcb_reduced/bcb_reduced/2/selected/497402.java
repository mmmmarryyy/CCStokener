...
public class ProcTaxaCambio extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        if (rate.getValidFrom().equals(AdempiereLBR.addDays(dataCambio, 1)) && rate.getMultiplyRate().equals(taxaVenda)) {
            return Msg.getMsg(Env.getAD_Language(getCtx()), "ProcessOK", true) + "<br/>Câmbio já atualizado.";
        }
        ...
        return Msg.getMsg(Env.getAD_Language(getCtx()), "ProcessOK", true) + "<br/>Data de Câmbio: " + dataCambio + "<br/> Taxa de Venda: " + taxaVenda;
    }
    ...
    private int getC_Conversion_Rate_ID(Timestamp validFrom) {
        String sql = "select c_conversion_rate_ID " + "from C_Conversion_Rate " + "where c_Currency_ID = ? " + "and c_Currency_ID_TO = ? " + "and validfrom = ? " + "and ad_Client_ID in (0,?) " + "order by AD_Client_ID desc";
        int C_Conversion_Rate_ID = DB.getSQLValue(get_TrxName(), sql, new Object[] { USD, BRL, validFrom, Env.getAD_Client_ID(getCtx()) });
