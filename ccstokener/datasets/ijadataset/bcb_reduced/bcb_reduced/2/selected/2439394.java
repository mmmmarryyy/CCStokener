...
public class NetInfoShenTie {
    ...
    public Vector getAllTrain() throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            if (line.equalsIgnoreCase("<TD ALIGN=CENTER VALIGN=CENTER BGCOLOR=#DFEFFF WIDTH=100 >")) count = 0;
            switch(count) {
                case 2:
                    fromName = line;
                    break;
    ...
    public Train getTrainInfo(STDB db, String fromTrainNo, int trainID) throws IOException, SQLException {
        ...
        } else url = URL_getTrainInfoA + fromTrainNo.trim();
        HttpURLConnection connection = connect(url);
        BufferedReader in = new BufferedReader(new InputStreamReader(connection.getInputStream()));
        String line;
        int status = 0;
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (line.endsWith("</TABLE>")) status = 2;
            if (status == 1) {
                buffer.append(line);
            }
        }
    ...
    private Train paserTrainInfoLine(STDB db, String line, int trainID) throws SQLException {
        ...
        while (remain.indexOf(trainInfoDataBeginPatten) >= 0) {
            ...
            switch(count % 8) {
                case 0:
                    ...
                    if (!trainNames.contains(trainNameCurrent)) trainNames.add(trainNameCurrent);
                    break;
                case 1:
                    break;
                case 2:
        ...
        if (theTrain.stopNum == 0) return null;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 131; i < fromNames.size(); i++) {
                ...
                if (theTrain != null) System.out.println(i + "> " + theTrain.getTrainName() + ": " + theTrain.getStartStation() + "-" + theTrain.getTerminalStation());
            }
            db.closeDB();
        } catch (IOException e) {
            e.printStackTrace();
