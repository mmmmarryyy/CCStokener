...
public class GroundspeakAPI {
    ...
    public static int getMyTbList(String accessToken, TbList list) {
        try {
            HttpGet httppost = new HttpGet(GS_LIVE_URL + "GetUsersTrackables?AccessToken=" + accessToken + "&StartIndex=0&MaxPerPage=30&TrackableLogCount=2&format=json");
            String result = Execute(httppost);
            try {
                JSONTokener tokener = new JSONTokener(result);
                JSONObject json = (JSONObject) tokener.nextValue();
    ...
    public static int getImagesForGeocache(String accessToken, String cacheCode, ArrayList<String> images) {
        try {
            HttpGet httppost = new HttpGet(GS_LIVE_URL + "GetImagesForGeocache?AccessToken=" + accessToken + "&CacheCode=" + cacheCode + "&format=json");
            String result = Execute(httppost);
            try {
                JSONTokener tokener = new JSONTokener(result);
                JSONObject json = (JSONObject) tokener.nextValue();
