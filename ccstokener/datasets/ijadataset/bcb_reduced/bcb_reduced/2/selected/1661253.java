 ...
 * @author  levan
 */
public class SRWGuiClient extends JFrame implements HyperlinkListener, ActionListener {

    public static void main(String[] args) {
        try {
            SRWSampleServiceLocator service = new SRWSampleServiceLocator();
            String urlString = "http://alcme.oclc.org/srw/search/SOAR";
            if (args.length == 1) urlString = args[0];
    ...
    public void actionPerformed(ActionEvent event) {
        String url;
        if (event.getSource() == urlField) url = urlField.getText(); else url = initialURL;
        try {
